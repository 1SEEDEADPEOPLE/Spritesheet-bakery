...
function setupColorGradeMaterial(mat, obj){
  if(mat.userData.colorGradeSetup) return; // idempotent — don't double-patch
  mat.userData.colorGradeSetup = true;
  mat.userData.colorGradeShaders = [];
  ...
  mat.onBeforeCompile = (shader) => {
    ...
    // IMPORTANT: this app renders the same scene through TWO separate
    // WebGLRenderer instances (the live viewport `renderer` and the
    // offscreen `captureRenderer` used for preview/manual-capture/bake).
    // Three.js compiles and caches shader programs PER RENDERER, so
    // onBeforeCompile fires once for each one that ends up drawing this
    // material — each call gets its OWN uniforms object. Overwriting a
    // single stored reference here means only whichever renderer compiled
    // it MOST RECENTLY would ever receive slider updates, leaving the
    // other stuck with stale (or default) values — exactly why a graded
    // object could look right in the live view but wrong in the bake, or
    // vice versa. Track every compiled shader and update all of them.
    mat.userData.colorGradeShaders.push(shader);
  };
...
function pushColorGradeUniforms(obj){
  obj.root.traverse(n=>{
    ...
    const mats = Array.isArray(n.material) ? n.material : [n.material];
    mats.forEach(m=>{
      (m.userData.colorGradeShaders||[]).forEach(shader=>{
        shader.uniforms.uBrightness.value = obj.brightness;
        shader.uniforms.uContrast.value = obj.contrast;
        shader.uniforms.uSaturation.value = obj.saturation;
      });
