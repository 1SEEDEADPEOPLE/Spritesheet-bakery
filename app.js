...
function updateCamStepperDisplays(){
  document.getElementById('cam-x-val').value=cam.position.x.toFixed(2);
  document.getElementById('cam-y-val').value=cam.position.y.toFixed(2);
  document.getElementById('cam-z-val').value=cam.position.z.toFixed(2);
...
document.getElementById('cam-x-val').addEventListener('change',e=>{cam.position.x=+e.target.value||0;controls.update();});
document.getElementById('cam-y-val').addEventListener('change',e=>{cam.position.y=+e.target.value||0;controls.update();});
document.getElementById('cam-z-val').addEventListener('change',e=>{cam.position.z=+e.target.value||0;controls.update();});
...
document.querySelectorAll('[data-camaxis]').forEach(btn=>btn.addEventListener('click',()=>{
  const dir=+btn.dataset.camdir, ax=btn.dataset.camaxis;
  if(ax==='x') cam.position.x+=camStep*dir;
  if(ax==='y') cam.position.y+=camStep*dir;
  if(ax==='z') cam.position.z+=camStep*dir;
  controls.update();
  updateCamStepperDisplays();
}));
...
function updateShadowCameraFrustum(){
  ...
  if(groundShadowEnabled) groundShadowCatcher.position.y = box.min.y - 0.005;
}
