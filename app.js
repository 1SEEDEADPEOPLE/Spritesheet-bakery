...
function updateShadowCameraFrustum(){
  ...
  const pad = maxDim*0.6 + 0.5;
  sun.shadow.camera.left = -pad;
  sun.shadow.camera.right = pad;
  sun.shadow.camera.top = pad;
  sun.shadow.camera.bottom = -pad;
  sun.target.position.copy(center);
  sun.target.updateMatrixWorld();
  // Near/far must be based on the ACTUAL current light-to-content distance,
  // not a fixed value — sun.position sits at a constant spherical offset
  // from the world origin, but once content moves or gets duplicated away
  // from origin (e.g. tiling scene objects into the distance), the real
  // distance from the light to that content grows well past the original
  // estimate, and a too-short far plane silently clips the shadow away
  // entirely — exactly the "flat, no shadow" symptom.
  const lightDist = sun.position.distanceTo(center);
  sun.shadow.camera.near = Math.max(0.05, lightDist - maxDim*2 - pad);
  sun.shadow.camera.far = lightDist + maxDim*2 + pad;
...
function onSceneObjectLoaded(name,blob,gltf,offset){
  try{
    ...
    const obj={
      ...
      anchorX:0,anchorY:0,anchorZ:0,joined:false,
      brightness:0,contrast:1,saturation:1
    };
...
function duplicateSceneObject(sourceId,offset){
  ...
  try{
    ...
    const obj={
      ...
      anchorX:src.anchorX??0,anchorY:src.anchorY??0,anchorZ:src.anchorZ??0,joined:src.joined??false,
  ...
  for(const d of dataArr){
    try{
      ...
      const obj={
        ...
        anchorX:d.anchorX??0,anchorY:d.anchorY??0,anchorZ:d.anchorZ??0,joined:d.joined??false,
