...
function renderSquareFrameAtRes(res, opts={}){
  ...
  scene.background=savedBG; grid.visible=savedGrid; anchorMarker.visible=savedMarker;
  r.dispose();
  return pix;
}
...
async function saveProjectAs(name){
  ...
  const rec={id, name:id, savedAt:Date.now(), glbBlob:currentGlbBlob, glbName:loadedName, settings:collectSettings()};
