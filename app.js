...
// ── Image planes: a PNG/JPG/etc. becomes a flat textured plane that's a
// scene object like any 3D model — same position/rotation/scale/duplicate
// controls, and it renders in preview/live-filter/bake automatically since
// it's just another child of the top-level scene. MeshBasicMaterial is
// unlit, so the image shows exactly as its source pixels regardless of the
// scene's lighting — correct for a flat asset, not something to be shaded.
...
function loadImageAsTexture(file){
  return new Promise((resolve,reject)=>{
    ...
    texLoader.load(url,tex=>{
      ...
      resolve({texture:tex,width:tex.image.width,height:tex.image.height});
    },undefined,err=>{URL.revokeObjectURL(url);reject(err);});
  });
}
function buildImagePlaneObject(texInfo){
  const aspect=texInfo.width/texInfo.height;
  let pw,ph;
  if(aspect>=1){pw=2;ph=2/aspect;} else {ph=2;pw=2*aspect;}
...
function loadSceneObjectFile(file){
  if(isImageFile(file.name)){
    loadImageAsTexture(file).then(texInfo=>onImagePlaneLoaded(file.name,file,texInfo,{}))
      .catch(err=>alert('Image load failed: '+(err.message||err)));
  } else {
...
function onImagePlaneLoaded(name,blob,texInfo,offset){
  const wrap=buildImagePlaneObject(texInfo);
  scene.add(wrap);
  const id=sceneObjIdCounter++;
  const obj={
    id,name,type:'image',imgBlob:blob,root:wrap,clips:[],mixer:null,
    selClipIdx:-1,animMode:'independent',phaseOffset:0,
...
function onSceneObjectLoaded(name,blob,gltf,offset){
  ...
  const id=sceneObjIdCounter++;
  const obj={
    id,name,type:'model',glbBlob:blob,root:wrap,clips:objClips,mixer:objMixer,
...
function duplicateSceneObject(sourceId,offset){
  ...
  const id=sceneObjIdCounter++;
  const obj={
    id,name:src.name+' copy',type:src.type,glbBlob:src.glbBlob,imgBlob:src.imgBlob,root:wrap,
...
function collectSceneObjectsForSave(){
  return sceneObjects.map(o=>({
    name:o.name, type:o.type||'model', glbBlob:o.glbBlob, imgBlob:o.imgBlob,
...
async function restoreSceneObjects(dataArr){
  ...
  for(const d of dataArr){
    try{
      if(d.type==='image'){
        const texInfo=await loadImageAsTexture(d.imgBlob);
        const wrap=buildImagePlaneObject(texInfo);
        scene.add(wrap);
        const id=sceneObjIdCounter++;
        const obj={
          id,name:d.name,type:'image',imgBlob:d.imgBlob,root:wrap,clips:[],mixer:null,
          selClipIdx:-1,animMode:d.animMode??'independent',phaseOffset:d.phaseOffset??0,
      ...
      const id=sceneObjIdCounter++;
      const obj={
        id,name:d.name,type:'model',glbBlob:d.glbBlob,root:wrap,clips:objClips,mixer:objMixer,
