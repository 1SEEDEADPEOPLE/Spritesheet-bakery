...
// Grouping: multiple scene objects can be clustered into an independent
// group with its own position/rotation/scale, applied on top of each
// member's own local transform. Multiple separate groups are supported.
let sceneGroups = [];
let groupIdCounter = 0;
let selectedGroupId = null;
let selectModeOn = false;
let selectedSceneObjIds = new Set();
let groupStep = 0.5, groupRotStep = 15;
...
let primaryScale = 1;
let primaryModelVisible = true;
let primaryVisible = true;
const PRIMARY_SCALE_MIN = 0.02, PRIMARY_SCALE_MAX = 50;
...
// Lighting: STUDIO (default 3-point rig) / FLAT (ambient-only, no directional
// highlight or shadow — right for flat ground/road tiles) / CUSTOM (manual sliders)
let lightPreset = 'studio';
let lightAmbient = 0.55, lightSun = 1.15, lightFill = 0.35, lightRim = 0.18, shadowsOn = true;
// Light rotation: sun/fill positions expressed as azimuth/elevation around
// the origin at a fixed radius, so "rotating" a light means moving it around
// a sphere rather than juggling raw XYZ — matches how a sun position is
// naturally thought about.
let sunAzimuth = 39, sunElevation = 55, sunRadius = 11;
let fillAzimuth = -143, fillElevation = 31, fillRadius = 5.8;
let lightRotStep = 15;
// Camera rig: FLY (free OrbitControls, default) vs LOCKED (precise numeric
// steppers, dragging disabled) — plus a list of saved camera bookmarks so a
// framing can be restored exactly after flying around to inspect something.
let camMode = 'fly';
let camStep = 0.5;
let savedShots = [];
let shotIdCounter = 0;
...
let filterType = 'none';
let pixelSize=48, pixelLevels=16, pixelOutline=false, pixelDither=false;
...
// Single persistent offscreen renderer, reused for manual captures, live filter
// preview, AND the main bake loop. Previously each capture/bake created a brand
// new WebGLRenderer (a new WebGL context) and disposed it afterward — mobile
// Safari has a hard cap on simultaneous live contexts, and rapid create/dispose
// cycles exhaust it faster than the browser reclaims them, which was crashing
// the whole page back to the upload screen after a few bakes. Reusing one
// context for the app's lifetime (alongside the main viewport's `renderer`)
// avoids that entirely.
const captureRenderer = new THREE.WebGLRenderer({antialias:true, alpha:true, preserveDrawingBuffer:true, premultipliedAlpha:false});
...
sun.castShadow=true;
sun.shadow.mapSize.set(512,512); // lower baseline — see setShadowQuality() below for why
...
updateLightPositions(); // sets sun/fill positions from the spherical state above
...
// Colored X/Y/Z axis gizmo shown on whichever scene object is currently
// selected, so it's obvious which way each axis actually points — matches
// the red/green/yellow used for the X/Y/Z labels in the position/rotation
// controls, so the two are easy to cross-reference at a glance.
function buildAxesGizmo(size){
  const group = new THREE.Group();
  const axes = [
    {dir:[1,0,0], color:0xff5c5c}, // X red
    {dir:[0,1,0], color:0x5cff8a}, // Y green
    {dir:[0,0,1], color:0xf5e05c}, // Z yellow
  ];
  axes.forEach(a=>{
    ...
    const mat = new THREE.LineBasicMaterial({color:a.color, depthTest:false, transparent:true});
...
const axesGizmo = buildAxesGizmo(0.6);
axesGizmo.visible = false;
function updateAxesGizmo(){
  if(axesGizmo.parent) axesGizmo.parent.remove(axesGizmo);
  const obj = getSelectedSceneObj();
  if(obj){
    obj.root.add(axesGizmo);
    axesGizmo.position.set(0,0,0);
    axesGizmo.visible = true;
  } else {
    axesGizmo.visible = false;
  }
}

// GROUND SHADOW — a large, otherwise-invisible plane using ShadowMaterial,
// which renders as nothing except a soft dark overlay exactly where a
// shadow falls on it. Solves models that have no actual ground/floor mesh
// in the scene (just the character alone) — a directional light has
// nothing to cast a shadow ONTO in that case, so the bake shows the
// subject floating with zero contact cue even though it's meant to be
// "standing." This plane gives the shadow something to land on without
// requiring the user to build an actual ground model. Positioned each time
// the shadow frustum recomputes, at the current lowest point of whatever's
// in the scene, so it tracks scale/position/pose changes automatically.
let groundShadowEnabled = false, groundShadowOpacity = 0.4;
...
function setGroundShadowEnabled(on){
  groundShadowEnabled = on;
  groundShadowCatcher.visible = on;
  updateGroundShadowCatcher();
}
function setGroundShadowOpacity(v){
  groundShadowOpacity = v;
  groundShadowCatcher.material.opacity = v;
}
// Repositions AND resizes the catcher from the scene's actual current
// bounds — needs to run every frame during animation, not just on discrete
// transform changes, since an idle/walk cycle moves the feet up and down
// continuously and a stale Y position is exactly what reads as "shadow
// floating below the model." Also fixes X/Z, which previously was never
// set at all (always sat at the world origin regardless of where the
// model actually was), and scales plane size with the model instead of a
// fixed size that clips once the model is scaled up.
function updateGroundShadowCatcher(){
  if(!groundShadowEnabled || !loadedScene) return;
  // precise=true is essential here, not just a nicety: Three.js's default
  // (fast) bounding box for a SkinnedMesh comes from geometry.boundingBox,
  // computed ONCE from the ORIGINAL bind-pose vertex data — it never
  // reflects the mesh's actual current animated pose no matter how often
  // this function runs. precise mode calls getVertexPosition() per vertex,
  // which for a SkinnedMesh applies the CURRENT bone transforms, giving
  // the real deformed position — this is what makes the shadow actually
  // track the feet through a pose/animation instead of sitting wherever
  // the bind pose happened to have them.
  const box = new THREE.Box3().setFromObject(pivot, true);
  sceneObjects.forEach(o=>{ if(o.root.visible) box.expandByObject(o.root, true); });
  if(box.isEmpty()) return;
  const size = box.getSize(new THREE.Vector3());
  const center = box.getCenter(new THREE.Vector3());
  const maxDim = Math.max(size.x, size.y, size.z, 0.5);
  const planeSize = Math.max(20, maxDim*12); // generous margin, scales with model
  groundShadowCatcher.scale.setScalar(planeSize);
  groundShadowCatcher.position.set(center.x, box.min.y-0.005, center.z);
}
...
function updateFrameGuide(){
  ...
  fg.style.left=left+'px'; fg.style.top=top+'px';
  // Live preview canvas overlays the SAME rectangle so this guide shows your
  // actual filtered/cropped output, not just an outline around the raw 3D view.
  const lc=document.getElementById('live-filter-canvas');
  lc.style.width=rw+'px'; lc.style.height=rh+'px';
  lc.style.left=left+'px'; lc.style.top=top+'px';
}
...
function tick(){
  ...
  if(groundShadowEnabled && mixer && liveAnim) updateGroundShadowCatcher();
  controls.update();
  if(isOrtho) syncOrtho();
  if(previewMode) updateFrameGuide();
  renderer.render(scene,cam);
  if(previewMode && loadedScene){
    const now=performance.now();
    if(now-lastPreviewMs>400){ lastPreviewMs=now; updateLivePreview(); }
  }
}
tick();
...
// ══════════════════════════════════════════════════════════════════════════
// CAMERA RIG — FLY/LOCKED mode + precise steppers + saved shots
// ══════════════════════════════════════════════════════════════════════════
// Camera orbit params are computed on-demand from cam.position/controls.target
// rather than stored separately, so they automatically stay correct after
// FLY-mode dragging, jumping to a saved shot, or anything else that moves
// the camera outside these controls.
function getCamOrbitParams(){
  ...
  const elevation=Math.asin(THREE.MathUtils.clamp(offset.y/distance,-1,1))*180/Math.PI;
  return {azimuth,elevation,distance};
}
function setCamOrbitParams(azimuth,elevation,distance){
  const az=THREE.MathUtils.degToRad(azimuth), el=THREE.MathUtils.degToRad(THREE.MathUtils.clamp(elevation,-89,89));
...
function updateCamStepperDisplays(){
  const {azimuth,elevation,distance}=getCamOrbitParams();
  document.getElementById('cam-azimuth-val').value=Math.round(azimuth);
  document.getElementById('cam-elevation-val').value=Math.round(elevation);
  document.getElementById('cam-distance-val').value=distance.toFixed(2);
  document.getElementById('look-x-val').value=controls.target.x.toFixed(2);
  document.getElementById('look-y-val').value=controls.target.y.toFixed(2);
  document.getElementById('look-z-val').value=controls.target.z.toFixed(2);
  document.getElementById('cam-zoom-slider').value=cam.zoom;
  document.getElementById('cam-zoom-val').textContent=cam.zoom.toFixed(2);
}
document.getElementById('cam-azimuth-val').addEventListener('change',e=>{
  const p=getCamOrbitParams(); setCamOrbitParams(+e.target.value||0,p.elevation,p.distance);
});
document.getElementById('cam-elevation-val').addEventListener('change',e=>{
  const p=getCamOrbitParams(); setCamOrbitParams(p.azimuth,+e.target.value||0,p.distance);
});
document.getElementById('cam-distance-val').addEventListener('change',e=>{
  const p=getCamOrbitParams(); setCamOrbitParams(p.azimuth,p.elevation,+e.target.value||p.distance);
});
document.getElementById('look-x-val').addEventListener('change',e=>{controls.target.x=+e.target.value||0;controls.update();});
document.getElementById('look-y-val').addEventListener('change',e=>{controls.target.y=+e.target.value||0;controls.update();});
document.getElementById('look-z-val').addEventListener('change',e=>{controls.target.z=+e.target.value||0;controls.update();});
document.getElementById('cam-mode-fly').addEventListener('click',()=>{
  camMode='fly';
  controls.enabled=true;
  document.getElementById('cam-mode-fly').classList.add('on');
  document.getElementById('cam-mode-locked').classList.remove('on');
  document.getElementById('cam-locked-controls').style.display='none';
});
document.getElementById('cam-mode-locked').addEventListener('click',()=>{
  camMode='locked';
  controls.enabled=false;
  document.getElementById('cam-mode-locked').classList.add('on');
  document.getElementById('cam-mode-fly').classList.remove('on');
  document.getElementById('cam-locked-controls').style.display='';
  updateCamStepperDisplays();
});
document.getElementById('cam-rotate-speed').addEventListener('input',e=>{
  controls.rotateSpeed=+e.target.value;
  document.getElementById('cam-rotate-speed-val').textContent=controls.rotateSpeed.toFixed(2);
});
document.querySelectorAll('[data-camstep]').forEach(b=>b.addEventListener('click',()=>{
  document.querySelectorAll('[data-camstep]').forEach(x=>x.classList.remove('on'));
  b.classList.add('on'); camStep=+b.dataset.camstep;
}));
document.querySelectorAll('[data-camorbit]').forEach(btn=>btn.addEventListener('click',()=>{
  const dir=+btn.dataset.camdir, param=btn.dataset.camorbit;
  const p=getCamOrbitParams();
  if(param==='azimuth') p.azimuth+=camStep*10*dir; // degrees — scale step up so it reads as a meaningful angular nudge
  if(param==='elevation') p.elevation+=camStep*10*dir;
  if(param==='distance') p.distance=Math.max(0.05,p.distance+camStep*dir);
  setCamOrbitParams(p.azimuth,p.elevation,p.distance);
  updateCamStepperDisplays();
}));
document.querySelectorAll('[data-lookaxis]').forEach(btn=>btn.addEventListener('click',()=>{
  const dir=+btn.dataset.camdir, ax=btn.dataset.lookaxis;
  if(ax==='x') controls.target.x+=camStep*dir;
  if(ax==='y') controls.target.y+=camStep*dir;
  if(ax==='z') controls.target.z+=camStep*dir;
  controls.update();
  updateCamStepperDisplays();
}));
document.getElementById('cam-zoom-slider').addEventListener('input',e=>{
  cam.zoom=+e.target.value;
  cam.updateProjectionMatrix();
  document.getElementById('cam-zoom-val').textContent=cam.zoom.toFixed(2);
});

function saveCameraShot(name){
  const shot={
    id:shotIdCounter++, name:name||`Shot ${savedShots.length+1}`,
    posX:cam.position.x,posY:cam.position.y,posZ:cam.position.z,
    tgtX:controls.target.x,tgtY:controls.target.y,tgtZ:controls.target.z,
    zoom:cam.zoom, isOrtho
  };
  savedShots.push(shot);
  renderShotsList();
}
function jumpToShot(id){
  const shot=savedShots.find(s=>s.id===id);
  if(!shot) return;
  if(shot.isOrtho!==isOrtho){
    isOrtho=shot.isOrtho; cam=isOrtho?orthoCam:perspCam;
    controls.object=cam;
    document.getElementById('cam-btn').textContent=isOrtho?'ORTHO':'PERSP';
    document.getElementById('cam-btn').classList.toggle('on',!isOrtho);
  }
  cam.position.set(shot.posX,shot.posY,shot.posZ);
  controls.target.set(shot.tgtX,shot.tgtY,shot.tgtZ);
  cam.zoom=shot.zoom;
  cam.updateProjectionMatrix();
  controls.update();
  syncOrtho();
...
function deleteShot(id){
  savedShots=savedShots.filter(s=>s.id!==id);
  renderShotsList();
}
function renderShotsList(){
  const list=document.getElementById('cam-shots-list');
  list.innerHTML='';
  if(!savedShots.length){ list.innerHTML='<div class="attach-empty">No saved shots yet</div>'; return; }
  savedShots.forEach(shot=>{
    ...
    const nm=document.createElement('div'); nm.className='attach-name'; nm.textContent=shot.name;
    const sub=document.createElement('div'); sub.className='attach-bone'; sub.textContent=shot.isOrtho?'ortho':'persp';
    info.appendChild(nm); info.appendChild(sub);
    row.appendChild(info);
    const del=document.createElement('button'); del.className='tbtn'; del.textContent='DEL';
    del.addEventListener('click',e=>{ e.stopPropagation(); deleteShot(shot.id); });
    row.appendChild(del);
    row.addEventListener('click',()=>jumpToShot(shot.id));
    list.appendChild(row);
  });
}
document.getElementById('save-shot-btn').addEventListener('click',()=>{
  const name=prompt('Name this camera shot:','');
  if(name===null) return; // cancelled
  saveCameraShot(name.trim());
});
...
// ══════════════════════════════════════════════════════════════════════════
// LIGHTING — STUDIO (default 3-point rig) / FLAT (ambient-only, no directional
// highlight or shadow) / CUSTOM (manual sliders). AmbientLight isn't affected
// by surface normals at all, so ambient-only lighting is genuinely flat —
// exactly right for a ground/road tile that shouldn't show a sun hotspot.
// ══════════════════════════════════════════════════════════════════════════
function applyLighting(){
  ambientLight.intensity=lightAmbient;
  sun.intensity=lightSun;
  fillLight.intensity=lightFill;
  rimLight.intensity=lightRim;
  sun.castShadow=shadowsOn;
  updateLightPositions();
}
function updateLightPositions(){
  const sunAz=THREE.MathUtils.degToRad(sunAzimuth), sunEl=THREE.MathUtils.degToRad(sunElevation);
  sun.position.set(
    sunRadius*Math.cos(sunEl)*Math.sin(sunAz),
    sunRadius*Math.sin(sunEl),
    sunRadius*Math.cos(sunEl)*Math.cos(sunAz)
  );
  const fillAz=THREE.MathUtils.degToRad(fillAzimuth), fillEl=THREE.MathUtils.degToRad(fillElevation);
  fillLight.position.set(
    fillRadius*Math.cos(fillEl)*Math.sin(fillAz),
    fillRadius*Math.sin(fillEl),
    fillRadius*Math.cos(fillEl)*Math.cos(fillAz)
  );
}
function updateLightRotDisplays(){
  document.getElementById('sun-az-val').value = sunAzimuth;
  document.getElementById('sun-el-val').value = sunElevation;
  document.getElementById('fill-az-val').value = fillAzimuth;
  document.getElementById('fill-el-val').value = fillElevation;
}
document.getElementById('sun-az-val').addEventListener('change',e=>{sunAzimuth=+e.target.value||0;setLightPresetBtn('custom');updateLightPositions();});
document.getElementById('sun-el-val').addEventListener('change',e=>{sunElevation=+e.target.value||0;setLightPresetBtn('custom');updateLightPositions();});
document.getElementById('fill-az-val').addEventListener('change',e=>{fillAzimuth=+e.target.value||0;setLightPresetBtn('custom');updateLightPositions();});
document.getElementById('fill-el-val').addEventListener('change',e=>{fillElevation=+e.target.value||0;setLightPresetBtn('custom');updateLightPositions();});
function updateLightDisplays(){
  document.getElementById('light-ambient').value=lightAmbient;
  document.getElementById('light-ambient-val').textContent=lightAmbient.toFixed(2);
  document.getElementById('light-sun').value=lightSun;
  document.getElementById('light-sun-val').textContent=lightSun.toFixed(2);
  document.getElementById('light-fill').value=lightFill;
  document.getElementById('light-fill-val').textContent=lightFill.toFixed(2);
  document.getElementById('light-rim').value=lightRim;
  document.getElementById('light-rim-val').textContent=lightRim.toFixed(2);
  document.getElementById('shadows-on-btn').classList.toggle('on',shadowsOn);
  document.getElementById('shadows-off-btn').classList.toggle('on',!shadowsOn);
  updateLightRotDisplays();
}
function setLightPresetBtn(preset){
  lightPreset=preset;
  document.getElementById('light-preset-studio').classList.toggle('on',preset==='studio');
  document.getElementById('light-preset-flat').classList.toggle('on',preset==='flat');
  document.getElementById('light-preset-custom').classList.toggle('on',preset==='custom');
}
document.getElementById('light-preset-studio').addEventListener('click',()=>{
  setLightPresetBtn('studio');
  lightAmbient=0.55; lightSun=1.15; lightFill=0.35; lightRim=0.18; shadowsOn=true;
  sunAzimuth=39; sunElevation=55; fillAzimuth=-143; fillElevation=31;
  applyLighting(); updateLightDisplays();
});
document.getElementById('light-preset-flat').addEventListener('click',()=>{
  setLightPresetBtn('flat');
  lightAmbient=1.0; lightSun=0; lightFill=0; lightRim=0; shadowsOn=false;
  applyLighting(); updateLightDisplays();
});
document.getElementById('light-preset-custom').addEventListener('click',()=>{
  setLightPresetBtn('custom');
});
document.querySelectorAll('[data-lrstep]').forEach(b=>b.addEventListener('click',()=>{
  document.querySelectorAll('[data-lrstep]').forEach(x=>x.classList.remove('on'));
  b.classList.add('on'); lightRotStep=+b.dataset.lrstep;
}));
document.querySelectorAll('[data-lraxis]').forEach(btn=>btn.addEventListener('click',()=>{
  const dir=+btn.dataset.lrdir, ax=btn.dataset.lraxis;
  if(ax==='sunaz') sunAzimuth+=lightRotStep*dir;
  if(ax==='sunel') sunElevation+=lightRotStep*dir;
  if(ax==='fillaz') fillAzimuth+=lightRotStep*dir;
  if(ax==='fillel') fillElevation+=lightRotStep*dir;
  setLightPresetBtn('custom');
  updateLightPositions();
  updateLightRotDisplays();
}));
document.getElementById('light-ambient').addEventListener('input',e=>{
  lightAmbient=+e.target.value; setLightPresetBtn('custom');
  applyLighting();
  document.getElementById('light-ambient-val').textContent=lightAmbient.toFixed(2);
});
document.getElementById('light-sun').addEventListener('input',e=>{
  lightSun=+e.target.value; setLightPresetBtn('custom');
  applyLighting();
  document.getElementById('light-sun-val').textContent=lightSun.toFixed(2);
});
document.getElementById('light-fill').addEventListener('input',e=>{
  lightFill=+e.target.value; setLightPresetBtn('custom');
  applyLighting();
  document.getElementById('light-fill-val').textContent=lightFill.toFixed(2);
});
document.getElementById('light-rim').addEventListener('input',e=>{
  lightRim=+e.target.value; setLightPresetBtn('custom');
  applyLighting();
  document.getElementById('light-rim-val').textContent=lightRim.toFixed(2);
});
document.getElementById('shadows-on-btn').addEventListener('click',()=>{
  shadowsOn=true; setLightPresetBtn('custom');
  applyLighting(); updateLightDisplays();
});
document.getElementById('shadows-off-btn').addEventListener('click',()=>{
  shadowsOn=false; setLightPresetBtn('custom');
  applyLighting(); updateLightDisplays();
});
document.getElementById('ground-shadow-on-btn').addEventListener('click',()=>{
  setGroundShadowEnabled(true);
  document.getElementById('ground-shadow-on-btn').classList.add('on');
  document.getElementById('ground-shadow-off-btn').classList.remove('on');
  document.getElementById('ground-shadow-opacity-row').style.display='';
});
document.getElementById('ground-shadow-off-btn').addEventListener('click',()=>{
  setGroundShadowEnabled(false);
  document.getElementById('ground-shadow-off-btn').classList.add('on');
  document.getElementById('ground-shadow-on-btn').classList.remove('on');
  document.getElementById('ground-shadow-opacity-row').style.display='none';
});
document.getElementById('ground-shadow-opacity').addEventListener('input',e=>{
  setGroundShadowOpacity(+e.target.value);
  document.getElementById('ground-shadow-opacity-val').textContent=(+e.target.value).toFixed(2);
});
...
document.getElementById('preview-btn').addEventListener('click',()=>{
  ...
  document.getElementById('live-filter-canvas').classList.toggle('show',previewMode);
  if(previewMode){
    gridVisBeforePreview=grid.visible;
    grid.visible=false;
    scene.background=null;
    updateFrameGuide();
    lastPreviewMs=0; // force an immediate refresh instead of waiting out the throttle
    updateLivePreview();
  } else {
...
function applyTransform(){
  ...
  pivot.scale.setScalar(primaryScale);
  pivot.rotation.set(
...
// The sun's shadow camera has a fixed-resolution shadow map, so its frustum
// needs to actually fit whatever's currently in the scene — otherwise a
// shrunk model (PRIMARY SCALE slider) reads as noisy "shadow acne" (the
// frustum is far too large relative to the object, so each shadow-map texel
// covers a huge area of it), and a model moved or scaled outside the old
// fixed bounds falls out of the shadow camera's range entirely and renders
// with no shadow at all — flat. Recomputed from the actual current bounds
// every time something moves/scales, instead of a value hardcoded for the
// original default size.
// Shadow map resolution is DELIBERATELY low by default (512) because the
// live viewport renders continuously at 60fps and the preview overlay
// refreshes every ~150ms — both pay the shadow-map cost on every single
// frame, and a scene with several props (each casting/receiving shadows)
// can add up to real memory/GPU pressure on a phone, which iOS Safari
// responds to by silently killing the page. The one-time BAKE operation is
// the only place full resolution actually matters, so it explicitly bumps
// up before rendering and restores the cheap baseline immediately after.
// Disposing the existing shadow.map forces Three.js to reallocate at the
// new size rather than possibly reusing a stale render target.
...
function updateShadowCameraFrustum(){
  ...
  sceneObjects.forEach(o=>{ if(o.root.visible) box.expandByObject(o.root); });
  if(box.isEmpty()) return;
  const center = box.getCenter(new THREE.Vector3());
  sun.target.position.copy(center);
  sun.target.updateMatrixWorld();

  // Fit the frustum tightly to the object's actual silhouette as seen from
  // the light, instead of a single "longest dimension" cube — a uniform
  // cube badly wastes shadow-map resolution on an elongated object (e.g. a
  // long, thin road tile), since its length would inflate the frustum's
  // width and height too, spreading the shadow map's texels far thinner
  // than the content actually needs and making shadows read as weak or
  // effectively invisible. Project the box corners into the light's own
  // right/up/forward axes and take the true min/max along each.
  const forward = new THREE.Vector3().subVectors(center, sun.position).normalize();
  let up = Math.abs(forward.y) > 0.99 ? new THREE.Vector3(1,0,0) : new THREE.Vector3(0,1,0);
  ...
  // The frustum also needs to contain the shadow itself, not just the
  // object casting it — at a low sun angle the shadow can stretch many
  // times the object's own height across the ground, well past its
  // bounding box. Project each of the box's TOP corners forward along the
  // light's travel direction until they hit ground level (the object's own
  // lowest point) and include those landing points too.
  if(forward.y < -0.001){
    ...
    topCorners.forEach(c=>{
      const t=(groundY-c.y)/forward.y;
      if(t>0) corners.push(new THREE.Vector3().copy(c).addScaledVector(forward,t));
    });
  }
  let minR=Infinity,maxR=-Infinity,minU=Infinity,maxU=-Infinity,minF=Infinity,maxF=-Infinity;
  const rel = new THREE.Vector3();
  corners.forEach(corner=>{
    rel.subVectors(corner, sun.position);
    const r=rel.dot(right), u=rel.dot(up), f=rel.dot(forward);
    if(r<minR)minR=r; if(r>maxR)maxR=r;
    if(u<minU)minU=u; if(u>maxU)maxU=u;
    if(f<minF)minF=f; if(f>maxF)maxF=f;
  });
  const pad = 0.1 + 0.05*Math.max(maxR-minR, maxU-minU, 0.1);
  sun.shadow.camera.left = minR - pad;
  sun.shadow.camera.right = maxR + pad;
  sun.shadow.camera.bottom = minU - pad;
  sun.shadow.camera.top = maxU + pad;
  sun.shadow.camera.near = Math.max(0.05, minF - pad);
  sun.shadow.camera.far = maxF + pad;
  sun.shadow.camera.updateProjectionMatrix();
  updateGroundShadowCatcher();
}
function updateRotDisplay(){
  document.getElementById('yaw-val').value = rotYaw;
  document.getElementById('pitch-val').value = rotPitch;
  document.getElementById('roll-val').value = rotRoll;
}
function updateAnchorDisplay(){
  document.getElementById('anchor-x-val').value=anchorX.toFixed(2);
  document.getElementById('anchor-y-val').value=anchorY.toFixed(2);
  document.getElementById('anchor-z-val').value=anchorZ.toFixed(2);
}
function updatePosDisplay(){
  document.getElementById('pos-x-val').value=posOffX.toFixed(2);
  document.getElementById('pos-y-val').value=posOffY.toFixed(2);
  document.getElementById('pos-z-val').value=posOffZ.toFixed(2);
  document.getElementById('primary-scale').value=scaleToSliderPos(primaryScale,PRIMARY_SCALE_MIN,PRIMARY_SCALE_MAX);
  document.getElementById('primary-scale-val').textContent=primaryScale.toFixed(2);
}
document.getElementById('primary-scale').addEventListener('input',e=>{
  primaryScale=sliderPosToScale(+e.target.value,PRIMARY_SCALE_MIN,PRIMARY_SCALE_MAX);
  applyTransform();
  document.getElementById('primary-scale-val').textContent=primaryScale.toFixed(2);
});
// Hides the primary model's own MESHES only — not the pivot/loadedScene
// container, and not the bones. Attachments are parented to bones deep
// inside loadedScene's skeleton (that's how they follow animation), and
// Three.js cascades .visible down the ancestor chain — so hiding the
// container would also hide every attachment. Toggling meshes individually
// keeps the skeleton "live" and leaves anything parented to it visible.
function setPrimaryModelVisible(visible){
  primaryModelVisible=visible;
  if(!loadedScene) return;
  loadedScene.traverse(n=>{ if(n.isMesh) n.visible=visible; });
}
document.getElementById('primary-vis-on').addEventListener('click',()=>{
  setPrimaryModelVisible(true);
  document.getElementById('primary-vis-on').classList.add('on');
  document.getElementById('primary-vis-off').classList.remove('on');
});
document.getElementById('primary-vis-off').addEventListener('click',()=>{
  setPrimaryModelVisible(false);
  document.getElementById('primary-vis-off').classList.add('on');
  document.getElementById('primary-vis-on').classList.remove('on');
});
document.getElementById('yaw-val').addEventListener('change',e=>{rotYaw=+e.target.value||0;applyTransform();});
document.getElementById('pitch-val').addEventListener('change',e=>{rotPitch=+e.target.value||0;applyTransform();});
document.getElementById('roll-val').addEventListener('change',e=>{rotRoll=+e.target.value||0;applyTransform();});
document.getElementById('anchor-x-val').addEventListener('change',e=>{anchorX=+e.target.value||0;applyTransform();});
document.getElementById('anchor-y-val').addEventListener('change',e=>{anchorY=+e.target.value||0;applyTransform();});
document.getElementById('anchor-z-val').addEventListener('change',e=>{anchorZ=+e.target.value||0;applyTransform();});
document.getElementById('pos-x-val').addEventListener('change',e=>{posOffX=+e.target.value||0;applyTransform();});
document.getElementById('pos-y-val').addEventListener('change',e=>{posOffY=+e.target.value||0;applyTransform();});
document.getElementById('pos-z-val').addEventListener('change',e=>{posOffZ=+e.target.value||0;applyTransform();});
...
document.getElementById('pos-reset').addEventListener('click',()=>{
  posOffX=posOffY=posOffZ=0; primaryScale=1; applyTransform(); updatePosDisplay();
});
...
// Scale sliders are logarithmic: the HTML range input holds a normalized
// 0..1 POSITION, converted to/from the actual scale value exponentially.
// With reciprocal min/max (e.g. 0.02 and 50), scale=1.0 lands exactly at
// slider position 0.5 — equal room to shrink or grow, instead of a linear
// slider where 1.0 sits almost against the left edge.
const ATTACH_SCALE_MIN=0.01, ATTACH_SCALE_MAX=100;
const SCENEOBJ_SCALE_MIN=0.02, SCENEOBJ_SCALE_MAX=50;
function scaleToSliderPos(scale,min,max){
  return Math.log(Math.max(min,Math.min(max,scale))/min) / Math.log(max/min);
}
function sliderPosToScale(pos,min,max){
  return min * Math.pow(max/min, pos);
}
function detectFormat(name){
  return (name||'').split('.').pop().toLowerCase();
}
// Format-aware loader shared by every load site in the app (primary model,
// attachments, scene objects, animation import, project restore). Returns a
// normalized {scene, animations} shape regardless of source format, so
// nothing downstream needs to know or care whether a file was a GLB, an FBX,
// or a plain image. Images resolve as a flat unlit textured plane mesh with
// animations:[] — since the primary-load path (onGLTFLoaded) and the scene
// object path (normalizeSceneObject) both just do generic bounding-box
// fit-and-center on whatever "scene" they're handed, a plane normalizes to
// the same 2-unit scale as any 3D model with zero special-casing needed.
function isImageFile(name){
  return ['png','jpg','jpeg','webp','gif','bmp'].includes(detectFormat(name));
}
function buildImagePlaneMesh(texInfo){
  // Built at its native aspect ratio, arbitrary base size — whichever caller
  // receives this (onGLTFLoaded or normalizeSceneObject) rescales it to the
  // standard 2-unit fit the same way it would for any other model.
  const aspect=texInfo.width/texInfo.height;
  const geo=new THREE.PlaneGeometry(aspect,1);
  const mat=new THREE.MeshBasicMaterial({map:texInfo.texture,transparent:true,side:THREE.DoubleSide});
  return new THREE.Mesh(geo,mat);
}
function loadGLBAsync(blob, nameHint){
  const name=nameHint||blob.name||'';
  ...
  return new Promise((resolve,reject)=>{
    ...
    if(ext==='fbx'){
      ...
      texLoader.load(url,tex=>{
        ...
        const mesh=buildImagePlaneMesh({texture:tex,width:tex.image.width,height:tex.image.height});
...
function onGLTFLoaded(name,gltf){
  ...
  primaryScale=1; primaryVisible=true;
  pivot.rotation.set(0,0,0);
  updateRotDisplay(); updatePosDisplay(); updateAnchorDisplay();

  pivot.add(loadedScene);
  setPrimaryModelVisible(primaryVisible);
  applyTransform();

  if(clips.length){ mixer=new THREE.AnimationMixer(loadedScene); mixer.timeScale=animSpeed; }
...
document.getElementById('clip-sel').addEventListener('change',e=>{playClip(+e.target.value); updateTargetFpsReadout();});
...
document.getElementById('frames-mode-count').addEventListener('click',()=>{
  framesMode='count';
  document.getElementById('frames-mode-count').classList.add('on');
  document.getElementById('frames-mode-fps').classList.remove('on');
  document.getElementById('frames-row').style.display='';
  document.getElementById('target-fps-row').style.display='none';
});
document.getElementById('frames-mode-fps').addEventListener('click',()=>{
  framesMode='fps';
  document.getElementById('frames-mode-fps').classList.add('on');
  document.getElementById('frames-mode-count').classList.remove('on');
  document.getElementById('frames-row').style.display='none';
  document.getElementById('target-fps-row').style.display='';
  updateTargetFpsReadout();
});
document.getElementById('target-fps').addEventListener('input',e=>{
  targetFps=Math.max(1,+e.target.value||30);
  updateTargetFpsReadout();
});
document.getElementById('anim-speed-slider').addEventListener('input',e=>{
  ...
  document.getElementById('anim-speed-val').textContent=animSpeed.toFixed(2)+'×';
  updateTargetFpsReadout();
});
document.getElementById('time-scrub').addEventListener('input',e=>{
  ...
  if(groundShadowEnabled) updateGroundShadowCatcher();
...
function renderAttachmentsList(){
  ...
  if(!attachments.length){
    ...
    updateSceneComplexityIndicator();
    return;
  }
  attachments.forEach(att=>{
    ...
    row.appendChild(info);
    const vis=document.createElement('button'); vis.className='tbtn'+(att.root.visible?' on':''); vis.textContent=att.root.visible?'ON':'OFF';
    vis.addEventListener('click',e=>{ e.stopPropagation(); att.root.visible=!att.root.visible; renderAttachmentsList(); updateShadowCameraFrustum(); });
    row.appendChild(vis);
    const del=document.createElement('button'); del.className='tbtn'; del.textContent='DEL';
...
function updateAttachDisplays(){
  ...
  document.getElementById('afpos-x-val').value=att.posX.toFixed(2);
  document.getElementById('afpos-y-val').value=att.posY.toFixed(2);
  document.getElementById('afpos-z-val').value=att.posZ.toFixed(2);
  document.getElementById('afrot-x-val').value = att.rotX;
  document.getElementById('afrot-y-val').value = att.rotY;
  document.getElementById('afrot-z-val').value = att.rotZ;
  document.getElementById('attach-scale').value=scaleToSliderPos(att.scale,ATTACH_SCALE_MIN,ATTACH_SCALE_MAX);
  document.getElementById('attach-scale-val').textContent=att.scale.toFixed(2);
  document.getElementById('attach-brightness').value=att.brightness;
  document.getElementById('attach-brightness-val').textContent=att.brightness.toFixed(2);
  document.getElementById('attach-contrast').value=att.contrast;
  document.getElementById('attach-contrast-val').textContent=att.contrast.toFixed(2);
  document.getElementById('attach-saturation').value=att.saturation;
  document.getElementById('attach-saturation-val').textContent=att.saturation.toFixed(2);
}
document.getElementById('attach-brightness').addEventListener('input',e=>{
  const att=getSelectedAttachment(); if(!att) return;
  att.brightness=+e.target.value; pushColorGradeUniforms(att);
  document.getElementById('attach-brightness-val').textContent=att.brightness.toFixed(2);
});
document.getElementById('attach-contrast').addEventListener('input',e=>{
  const att=getSelectedAttachment(); if(!att) return;
  att.contrast=+e.target.value; pushColorGradeUniforms(att);
  document.getElementById('attach-contrast-val').textContent=att.contrast.toFixed(2);
});
document.getElementById('attach-saturation').addEventListener('input',e=>{
  const att=getSelectedAttachment(); if(!att) return;
  att.saturation=+e.target.value; pushColorGradeUniforms(att);
  document.getElementById('attach-saturation-val').textContent=att.saturation.toFixed(2);
});
document.getElementById('attach-grade-reset').addEventListener('click',()=>{
  const att=getSelectedAttachment(); if(!att) return;
  att.brightness=0; att.contrast=1; att.saturation=1;
  pushColorGradeUniforms(att); updateAttachDisplays();
});
document.getElementById('afpos-x-val').addEventListener('change',e=>{const att=getSelectedAttachment();if(!att)return;att.posX=+e.target.value||0;applyAttachmentTransform(att);});
document.getElementById('afpos-y-val').addEventListener('change',e=>{const att=getSelectedAttachment();if(!att)return;att.posY=+e.target.value||0;applyAttachmentTransform(att);});
document.getElementById('afpos-z-val').addEventListener('change',e=>{const att=getSelectedAttachment();if(!att)return;att.posZ=+e.target.value||0;applyAttachmentTransform(att);});
document.getElementById('afrot-x-val').addEventListener('change',e=>{const att=getSelectedAttachment();if(!att)return;att.rotX=+e.target.value||0;applyAttachmentTransform(att);});
document.getElementById('afrot-y-val').addEventListener('change',e=>{const att=getSelectedAttachment();if(!att)return;att.rotY=+e.target.value||0;applyAttachmentTransform(att);});
document.getElementById('afrot-z-val').addEventListener('change',e=>{const att=getSelectedAttachment();if(!att)return;att.rotZ=+e.target.value||0;applyAttachmentTransform(att);});
...
document.getElementById('attach-scale').addEventListener('input',e=>{
  const att=getSelectedAttachment(); if(!att) return;
  att.scale=sliderPosToScale(+e.target.value,ATTACH_SCALE_MIN,ATTACH_SCALE_MAX);
  applyAttachmentTransform(att);
  document.getElementById('attach-scale-val').textContent=att.scale.toFixed(2);
});
  ...
  for(const ad of attachmentsData){
    try{
      ...
      const att={id,name:ad.name,glbBlob:ad.glbBlob,root,boneName:null,
        ...
        brightness:ad.brightness??0,contrast:ad.contrast??1,saturation:ad.saturation??1};
      setupColorGradeForObject(att);
...
function collectAttachmentsForSave(){
  return attachments.map(a=>({
    ...
    scale:a.scale, visible:a.root.visible,
...
// ══════════════════════════════════════════════════════════════════════════
// ══════════════════════════════════════════════════════════════════════════
// PER-OBJECT COLOR GRADE — brightness/contrast/saturation applied to a single
// attachment or scene object, independent of everything else in the scene.
// Implemented via Material.onBeforeCompile, injecting a small grading step
// into each material's fragment shader right before its dithering chunk
// (i.e. operating on the final display-space color, same stage the EXPOSURE
// slider and 2D filters already work in — so it behaves the way a brightness/
// contrast/saturation slider intuitively should, not like a raw-light edit).
// Uniform VALUES update live without recompiling; only first use compiles.
// ══════════════════════════════════════════════════════════════════════════
// Shader references and setup state are tracked here, deliberately OUTSIDE
// any Three.js object's userData. Material.copy() (invoked by every
// .clone()) unconditionally does `this.userData = JSON.parse(JSON.stringify(
// source.userData))` internally — confirmed straight from the Three.js
// source. Storing raw shader objects (which contain circular references)
// in userData means ANY future .clone() of that material — anywhere, for
// any reason — throws inside Three.js's own code before our try/catch
// blocks even get a chance to run cleanly. A WeakMap the library never
// touches sidesteps this entirely. It also means onBeforeCompile itself
// isn't copied by .clone() (confirmed from source — copy() doesn't touch
// it), so a duplicated material needs re-patching, not just re-tracking.
const colorGradeSetupMats = new WeakSet();
const colorGradeShaderMap = new WeakMap();
function setupColorGradeMaterial(mat, obj){
  if(colorGradeSetupMats.has(mat)) return; // idempotent — don't double-patch
  colorGradeSetupMats.add(mat);
  colorGradeShaderMap.set(mat, []);
  const prevOnBeforeCompile = mat.onBeforeCompile;
  mat.onBeforeCompile = (shader) => {
    if(prevOnBeforeCompile) prevOnBeforeCompile(shader);
    shader.uniforms.uBrightness = { value: obj.brightness };
    shader.uniforms.uContrast   = { value: obj.contrast };
    shader.uniforms.uSaturation = { value: obj.saturation };
    ...
    shader.fragmentShader = shader.fragmentShader.replace(
      ...
      {
        float cgLuma = dot(gl_FragColor.rgb, vec3(0.299,0.587,0.114));
        gl_FragColor.rgb = mix(vec3(cgLuma), gl_FragColor.rgb, uSaturation);
      }
      gl_FragColor.rgb = clamp(gl_FragColor.rgb, 0.0, 1.0);
      #include <dithering_fragment>
      `
    );
    // This app renders the same scene through TWO separate WebGLRenderer
    // instances (the live viewport `renderer` and the offscreen
    // `captureRenderer` used for preview/manual-capture/bake). Three.js
    // compiles and caches shader programs PER RENDERER, so onBeforeCompile
    // fires once for each one that ends up drawing this material — each
    // call gets its OWN uniforms object. Track every compiled shader (not
    // just the most recent) so slider updates reach all of them.
    colorGradeShaderMap.get(mat).push(shader);
  };
  mat.needsUpdate = true;
}
function setupColorGradeForObject(obj){
  obj.root.traverse(n=>{
    if(!n.material) return;
    const mats = Array.isArray(n.material) ? n.material : [n.material];
    mats.forEach(m=>setupColorGradeMaterial(m, obj));
  });
}
function pushColorGradeUniforms(obj){
  obj.root.traverse(n=>{
    if(!n.material) return;
    const mats = Array.isArray(n.material) ? n.material : [n.material];
    mats.forEach(m=>{
      (colorGradeShaderMap.get(m)||[]).forEach(shader=>{
        shader.uniforms.uBrightness.value = obj.brightness;
        shader.uniforms.uContrast.value = obj.contrast;
        shader.uniforms.uSaturation.value = obj.saturation;
      });
...
// Materials are shared by reference after SkeletonUtils.clone() — deep-clone
// them so a duplicate's color grade doesn't also repaint the original (or
// vice versa), then set up (fresh) grading on the independent copies.
function cloneMaterialsForIndependence(root){
  root.traverse(n=>{
    if(!n.material) return;
...
function applySceneObjTransform(obj){
  const anchor = new THREE.Vector3(obj.anchorX||0, obj.anchorY||0, obj.anchorZ||0);
  obj.root.position.set(obj.posX,obj.posY,obj.posZ).add(anchor);
  if(obj.root.children[0]) obj.root.children[0].position.copy(anchor).multiplyScalar(-1);
...
// JOIN TO PARENT: re-parents the object's root between the top-level scene
// (independent — today's default) and the primary model's pivot (moves,
// rotates, and scales together with it). obj.posX/Y/Z etc. stay the same
// numbers either way — Three.js interprets them as LOCAL to whichever
// parent currently holds the object, so switching parents is all that's
// needed for the "moves with the whole scene" behavior.
function setSceneObjJoined(obj, joined){
  obj.joined = joined;
  const targetParent = joined ? pivot : scene;
  if(obj.root.parent !== targetParent) targetParent.add(obj.root);
}
function getSelectedSceneObj(){ return sceneObjects.find(o=>o.id===selectedSceneObjId)||null; }

// ══════════════════════════════════════════════════════════════════════════
// GROUPING — cluster several scene objects so they move/rotate/scale
// together as one unit, while each member keeps its own individual
// position/rotation/scale relative to the group underneath. Multiple
// independent groups are supported.
// ══════════════════════════════════════════════════════════════════════════
function getSelectedSceneGroup(){ return sceneGroups.find(g=>g.id===selectedGroupId)||null; }
// After .attach() reparents an object while preserving its world position,
// Three.js recomputes the object's ACTUAL local position/rotation/scale —
// but that doesn't touch our own posX/posY/posZ/rotX/rotY/rotZ/scale state,
// which is what actually gets saved/exported. Left out of sync, a grouped
// (or ungrouped) object keeps looking right on screen but silently saves
// its PRE-grouping coordinates — exactly why a reloaded project could show
// objects back where they were before you organized them into groups.
function syncSceneObjStateFromTransform(obj){
  const anchor = new THREE.Vector3(obj.anchorX||0, obj.anchorY||0, obj.anchorZ||0);
  const pos = obj.root.position.clone().sub(anchor);
  obj.posX = pos.x; obj.posY = pos.y; obj.posZ = pos.z;
  obj.rotX = Math.round(THREE.MathUtils.radToDeg(obj.root.rotation.x)*100)/100;
  obj.rotY = Math.round(THREE.MathUtils.radToDeg(obj.root.rotation.y)*100)/100;
  obj.rotZ = Math.round(THREE.MathUtils.radToDeg(obj.root.rotation.z)*100)/100;
  obj.scale = obj.root.scale.x; // uniform scale — x/y/z always equal
}
function createGroupFromSelection(){
  const ids=[...selectedSceneObjIds];
  if(ids.length<2){ alert('Select at least 2 objects to group (tap SELECT, check some objects, then group).'); return; }
  const members=sceneObjects.filter(o=>ids.includes(o.id));
  const root=new THREE.Group();
  scene.add(root);
  // .attach() reparents while preserving each object's CURRENT world
  // transform — nothing visually jumps at the moment of grouping, unlike
  // .add() which would reinterpret its existing local values relative to
  // the new parent.
  members.forEach(o=>{ root.attach(o.root); syncSceneObjStateFromTransform(o); });
  const id=groupIdCounter++;
  const group={
    id, name:`Group ${sceneGroups.length+1}`, root,
    posX:0,posY:0,posZ:0, rotX:0,rotY:0,rotZ:0, scale:1,
    memberIds: ids
  };
  members.forEach(o=>{ o.groupId=id; });
  sceneGroups.push(group);
  selectedGroupId=id;
  selectedSceneObjId=null;
  selectedSceneObjIds.clear();
  selectModeOn=false;
  document.getElementById('scene-obj-select-off').classList.add('on');
  document.getElementById('scene-obj-select-on').classList.remove('on');
  renderSceneObjList();
  renderSceneGroupsList();
  renderSceneObjEditor();
  renderSceneGroupEditor();
}
function applySceneGroupTransform(group){
  group.root.position.set(group.posX,group.posY,group.posZ);
  group.root.rotation.set(
    THREE.MathUtils.degToRad(group.rotX),
    THREE.MathUtils.degToRad(group.rotY),
    THREE.MathUtils.degToRad(group.rotZ)
  );
  group.root.scale.setScalar(group.scale);
  updateShadowCameraFrustum();
}
function ungroupSceneGroup(id){
  const group=sceneGroups.find(g=>g.id===id);
  if(!group) return;
  const members=sceneObjects.filter(o=>o.groupId===id);
  members.forEach(o=>{
    scene.attach(o.root); // preserves current world position — no snapping
    syncSceneObjStateFromTransform(o);
    o.groupId=null;
  });
  scene.remove(group.root);
  sceneGroups=sceneGroups.filter(g=>g.id!==id);
...
function deleteSceneGroup(id){
  const group=sceneGroups.find(g=>g.id===id);
  if(!group) return;
  const memberIds=sceneObjects.filter(o=>o.groupId===id).map(o=>o.id);
  memberIds.forEach(mid=>removeSceneObject(mid));
  scene.remove(group.root);
  sceneGroups=sceneGroups.filter(g=>g.id!==id);
  if(selectedGroupId===id) selectedGroupId=null;
  renderSceneObjList();
...
function updateGroupSelectedRow(){
  const row=document.getElementById('group-selected-row');
  const count=selectedSceneObjIds.size;
  row.style.display = (selectModeOn && count>0) ? '' : 'none';
  document.getElementById('group-selected-count').textContent=count;
}
function renderSceneGroupsList(){
  const list=document.getElementById('scene-groups-list');
  list.innerHTML='';
  if(!sceneGroups.length){ list.innerHTML='<div class="attach-empty">No groups yet</div>'; return; }
  sceneGroups.forEach(group=>{
    const members=sceneObjects.filter(o=>o.groupId===group.id);
    const row=document.createElement('div');
    row.className='attach-row'+(group.id===selectedGroupId?' selected':'');
    const info=document.createElement('div'); info.className='attach-info';
    const nm=document.createElement('div'); nm.className='attach-name'; nm.textContent=group.name;
    const sub=document.createElement('div'); sub.className='attach-bone';
    sub.textContent=`${members.length} object(s): `+members.map(m=>m.name).join(', ');
    info.appendChild(nm); info.appendChild(sub);
    row.appendChild(info);
    const vis=document.createElement('button'); vis.className='tbtn'+(group.root.visible?' on':''); vis.textContent=group.root.visible?'ON':'OFF';
    vis.addEventListener('click',e=>{ e.stopPropagation(); group.root.visible=!group.root.visible; renderSceneGroupsList(); updateShadowCameraFrustum(); });
    row.appendChild(vis);
    row.addEventListener('click',()=>{
...
function updateSceneGroupDisplays(){
  const group=getSelectedSceneGroup(); if(!group) return;
  document.getElementById('grp-pos-x-val').value=group.posX.toFixed(2);
  document.getElementById('grp-pos-y-val').value=group.posY.toFixed(2);
  document.getElementById('grp-pos-z-val').value=group.posZ.toFixed(2);
  document.getElementById('grp-rot-x-val').value=group.rotX;
  document.getElementById('grp-rot-y-val').value=group.rotY;
  document.getElementById('grp-rot-z-val').value=group.rotZ;
  document.getElementById('scene-group-scale').value=scaleToSliderPos(group.scale,SCENEOBJ_SCALE_MIN,SCENEOBJ_SCALE_MAX);
  document.getElementById('scene-group-scale-val').textContent=group.scale.toFixed(2);
  document.getElementById('scene-group-vis-on').classList.toggle('on',group.root.visible);
  document.getElementById('scene-group-vis-off').classList.toggle('on',!group.root.visible);
  const members=sceneObjects.filter(o=>o.groupId===group.id);
  document.getElementById('scene-group-members').textContent=members.map(m=>m.name).join(', ');
}
function renderSceneGroupEditor(){
  const group=getSelectedSceneGroup();
  const editor=document.getElementById('scene-group-editor');
  if(!group){ editor.style.display='none'; return; }
  editor.style.display='';
  updateSceneGroupDisplays();
}

// Scene objects are unified through loadGLBAsync (see above) — a PNG/JPG
// loads as a plane mesh, a GLB/FBX loads as its own scene graph, and both
// come back in the same {scene, animations} shape, so this one loader/
// normalizer pair handles every case with no branching needed here.
...
function onSceneObjectLoaded(name,blob,gltf,offset){
  try{
    ...
    const obj={
      id,name,type:isImageFile(name)?'image':'model',glbBlob:blob,root:wrap,clips:objClips,mixer:objMixer,
      ...
      anchorX:0,anchorY:0,anchorZ:0,joined:false,groupId:null,
      brightness:0,contrast:1,saturation:1
    };
    setupColorGradeForObject(obj);
    ...
    console.error('onSceneObjectLoaded failed:',e);
    alert('Failed to add scene object: '+(e.message||e));
  }
}
function duplicatePrimaryModel(offset){
  ...
  try{
    ...
    cloneMaterialsForIndependence(cloneScene);
    const wrap=new THREE.Group();
    wrap.add(cloneScene);
    scene.add(wrap);
    ...
    const obj={
      id,name:(loadedName||'Model')+' copy',type:'model',glbBlob:currentGlbBlob,root:wrap,
    ...
    console.error('duplicatePrimaryModel failed:',e);
    alert('Failed to duplicate model: '+(e.message||e));
  }
}
function duplicateSceneObject(sourceId,offset){
  ...
  try{
    ...
    const id=sceneObjIdCounter++;
    const obj={
      id,name:src.name+' copy',type:src.type,glbBlob:src.glbBlob,root:wrap,
      ...
      anchorX:src.anchorX??0,anchorY:src.anchorY??0,anchorZ:src.anchorZ??0,joined:src.joined??false,groupId:null,
      brightness:src.brightness??0,contrast:src.contrast??1,saturation:src.saturation??1
    };
    setupColorGradeForObject(obj);
    if(obj.joined) setSceneObjJoined(obj,true);
    applySceneObjTransform(obj);
    ...
    console.error('duplicateSceneObject failed:',e);
    alert('Failed to duplicate object: '+(e.message||e));
  }
}
function removeSceneObject(id){
  const obj=sceneObjects.find(o=>o.id===id);
  if(obj){ if(obj.root.parent) obj.root.parent.remove(obj.root); if(obj.mixer) obj.mixer.stopAllAction(); }
...
function clearAllSceneObjects(){
  ...
  sceneObjects=[];
  sceneGroups.forEach(g=>scene.remove(g.root));
  sceneGroups=[];
  selectedSceneObjId=null;
...
// Rough, cheap heuristic (visible prop count) rather than an actual
// triangle/memory count — the goal is just to flag "this scene now has
// several full 3D props in it" as a heads-up before PREVIEW or BAKE, since
// that's the situation most likely to strain a phone's memory/GPU budget.
function updateSceneComplexityIndicator(){
  const el=document.getElementById('scene-complexity-indicator');
  if(!el) return;
  const total=sceneObjects.filter(o=>o.root.visible).length+attachments.filter(a=>a.root.visible).length;
  el.className='';
  if(total===0){ el.textContent=''; return; }
  if(total<=2){ el.textContent=`${total} visible`; el.classList.add('complexity-ok'); }
  else if(total<=5){ el.textContent=`${total} visible · moderate`; el.classList.add('complexity-warn'); }
  else { el.textContent=`${total} visible · heavy, hide unused before preview/bake`; el.classList.add('complexity-bad'); }
}
function renderSceneObjList(){
  ...
  if(!ungrouped.length){ list.innerHTML='<div class="attach-empty">No scene objects yet</div>'; updateGroupSelectedRow(); updateSceneComplexityIndicator(); return; }
  ungrouped.forEach(obj=>{
    ...
    const sub=document.createElement('div'); sub.className='attach-bone';
    sub.textContent = obj.type==='image' ? 'image' : (obj.clips.length ? `${obj.clips.length} clip(s) · ${obj.animMode}` : 'static');
    info.appendChild(nm); info.appendChild(sub);
    row.appendChild(info);
    if(selectModeOn){
      const chk=document.createElement('button');
      chk.className='tbtn'+(selectedSceneObjIds.has(obj.id)?' on':'');
      chk.textContent=selectedSceneObjIds.has(obj.id)?'✓':'—';
      chk.addEventListener('click',e=>{
        e.stopPropagation();
        if(selectedSceneObjIds.has(obj.id)) selectedSceneObjIds.delete(obj.id);
        else selectedSceneObjIds.add(obj.id);
        renderSceneObjList();
      });
      row.appendChild(chk);
      row.addEventListener('click',()=>chk.click());
    } else {
      const vis=document.createElement('button'); vis.className='tbtn'+(obj.root.visible?' on':''); vis.textContent=obj.root.visible?'ON':'OFF';
      vis.addEventListener('click',e=>{ e.stopPropagation(); obj.root.visible=!obj.root.visible; renderSceneObjList(); updateShadowCameraFrustum(); });
      row.appendChild(vis);
      const del=document.createElement('button'); del.className='tbtn'; del.textContent='DEL';
  ...
  updateGroupSelectedRow();
  updateSceneComplexityIndicator();
}
function updateSceneObjDisplays(){
  const obj=getSelectedSceneObj(); if(!obj) return;
  document.getElementById('so-pos-x-val').value=obj.posX.toFixed(2);
  document.getElementById('so-pos-y-val').value=obj.posY.toFixed(2);
  document.getElementById('so-pos-z-val').value=obj.posZ.toFixed(2);
  document.getElementById('so-rot-x-val').value = obj.rotX;
  document.getElementById('so-rot-y-val').value = obj.rotY;
  document.getElementById('so-rot-z-val').value = obj.rotZ;
  document.getElementById('scene-obj-scale').value=scaleToSliderPos(obj.scale,SCENEOBJ_SCALE_MIN,SCENEOBJ_SCALE_MAX);
  document.getElementById('scene-obj-scale-val').textContent=obj.scale.toFixed(2);
  document.getElementById('so-anchor-x-val').value=(obj.anchorX||0).toFixed(2);
  document.getElementById('so-anchor-y-val').value=(obj.anchorY||0).toFixed(2);
  document.getElementById('so-anchor-z-val').value=(obj.anchorZ||0).toFixed(2);
  document.getElementById('scene-obj-parent-indep').classList.toggle('on',!obj.joined);
  document.getElementById('scene-obj-parent-joined').classList.toggle('on',!!obj.joined);
  ...
  document.getElementById('scene-obj-brightness').value=obj.brightness;
  document.getElementById('scene-obj-brightness-val').textContent=obj.brightness.toFixed(2);
  document.getElementById('scene-obj-contrast').value=obj.contrast;
  document.getElementById('scene-obj-contrast-val').textContent=obj.contrast.toFixed(2);
  document.getElementById('scene-obj-saturation').value=obj.saturation;
  document.getElementById('scene-obj-saturation-val').textContent=obj.saturation.toFixed(2);
}
document.getElementById('scene-obj-brightness').addEventListener('input',e=>{
  const obj=getSelectedSceneObj(); if(!obj) return;
  obj.brightness=+e.target.value; pushColorGradeUniforms(obj);
  document.getElementById('scene-obj-brightness-val').textContent=obj.brightness.toFixed(2);
});
document.getElementById('scene-obj-contrast').addEventListener('input',e=>{
  const obj=getSelectedSceneObj(); if(!obj) return;
  obj.contrast=+e.target.value; pushColorGradeUniforms(obj);
  document.getElementById('scene-obj-contrast-val').textContent=obj.contrast.toFixed(2);
});
document.getElementById('scene-obj-saturation').addEventListener('input',e=>{
  const obj=getSelectedSceneObj(); if(!obj) return;
  obj.saturation=+e.target.value; pushColorGradeUniforms(obj);
  document.getElementById('scene-obj-saturation-val').textContent=obj.saturation.toFixed(2);
});
document.getElementById('scene-obj-grade-reset').addEventListener('click',()=>{
  const obj=getSelectedSceneObj(); if(!obj) return;
  obj.brightness=0; obj.contrast=1; obj.saturation=1;
  pushColorGradeUniforms(obj); updateSceneObjDisplays();
});
document.getElementById('so-pos-x-val').addEventListener('change',e=>{const obj=getSelectedSceneObj();if(!obj)return;obj.posX=+e.target.value||0;applySceneObjTransform(obj);});
document.getElementById('so-pos-y-val').addEventListener('change',e=>{const obj=getSelectedSceneObj();if(!obj)return;obj.posY=+e.target.value||0;applySceneObjTransform(obj);});
document.getElementById('so-pos-z-val').addEventListener('change',e=>{const obj=getSelectedSceneObj();if(!obj)return;obj.posZ=+e.target.value||0;applySceneObjTransform(obj);});
document.getElementById('so-rot-x-val').addEventListener('change',e=>{const obj=getSelectedSceneObj();if(!obj)return;obj.rotX=+e.target.value||0;applySceneObjTransform(obj);});
document.getElementById('so-rot-y-val').addEventListener('change',e=>{const obj=getSelectedSceneObj();if(!obj)return;obj.rotY=+e.target.value||0;applySceneObjTransform(obj);});
document.getElementById('so-rot-z-val').addEventListener('change',e=>{const obj=getSelectedSceneObj();if(!obj)return;obj.rotZ=+e.target.value||0;applySceneObjTransform(obj);});
function renderSceneObjEditor(){
  const obj=getSelectedSceneObj();
  const editor=document.getElementById('scene-obj-editor');
  if(!obj){ editor.style.display='none'; updateAxesGizmo(); return; }
  editor.style.display='';
...
document.getElementById('scene-obj-select-off').addEventListener('click',()=>{
  selectModeOn=false; selectedSceneObjIds.clear();
  document.getElementById('scene-obj-select-off').classList.add('on');
  document.getElementById('scene-obj-select-on').classList.remove('on');
  renderSceneObjList();
});
document.getElementById('scene-obj-select-on').addEventListener('click',()=>{
  selectModeOn=true;
  document.getElementById('scene-obj-select-on').classList.add('on');
  document.getElementById('scene-obj-select-off').classList.remove('on');
  renderSceneObjList();
});
document.getElementById('group-selected-btn').addEventListener('click',createGroupFromSelection);
document.querySelectorAll('[data-grpstep]').forEach(b=>b.addEventListener('click',()=>{
  document.querySelectorAll('[data-grpstep]').forEach(x=>x.classList.remove('on'));
  b.classList.add('on'); groupStep=+b.dataset.grpstep;
}));
document.querySelectorAll('[data-grprstep]').forEach(b=>b.addEventListener('click',()=>{
  document.querySelectorAll('[data-grprstep]').forEach(x=>x.classList.remove('on'));
  b.classList.add('on'); groupRotStep=+b.dataset.grprstep;
}));
document.querySelectorAll('[data-grpaxis]').forEach(btn=>btn.addEventListener('click',()=>{
  const group=getSelectedSceneGroup(); if(!group) return;
  const dir=+btn.dataset.dir, ax=btn.dataset.grpaxis;
  if(ax==='x') group.posX=+((group.posX+groupStep*dir).toFixed(3));
  if(ax==='y') group.posY=+((group.posY+groupStep*dir).toFixed(3));
  if(ax==='z') group.posZ=+((group.posZ+groupStep*dir).toFixed(3));
  applySceneGroupTransform(group); updateSceneGroupDisplays();
}));
document.querySelectorAll('[data-grprxis]').forEach(btn=>btn.addEventListener('click',()=>{
  const group=getSelectedSceneGroup(); if(!group) return;
  const dir=+btn.dataset.dir, ax=btn.dataset.grprxis;
  if(ax==='x') group.rotX+=groupRotStep*dir;
  if(ax==='y') group.rotY+=groupRotStep*dir;
  if(ax==='z') group.rotZ+=groupRotStep*dir;
  applySceneGroupTransform(group); updateSceneGroupDisplays();
}));
document.getElementById('grp-pos-x-val').addEventListener('change',e=>{const g=getSelectedSceneGroup();if(!g)return;g.posX=+e.target.value||0;applySceneGroupTransform(g);});
document.getElementById('grp-pos-y-val').addEventListener('change',e=>{const g=getSelectedSceneGroup();if(!g)return;g.posY=+e.target.value||0;applySceneGroupTransform(g);});
document.getElementById('grp-pos-z-val').addEventListener('change',e=>{const g=getSelectedSceneGroup();if(!g)return;g.posZ=+e.target.value||0;applySceneGroupTransform(g);});
document.getElementById('grp-rot-x-val').addEventListener('change',e=>{const g=getSelectedSceneGroup();if(!g)return;g.rotX=+e.target.value||0;applySceneGroupTransform(g);});
document.getElementById('grp-rot-y-val').addEventListener('change',e=>{const g=getSelectedSceneGroup();if(!g)return;g.rotY=+e.target.value||0;applySceneGroupTransform(g);});
document.getElementById('grp-rot-z-val').addEventListener('change',e=>{const g=getSelectedSceneGroup();if(!g)return;g.rotZ=+e.target.value||0;applySceneGroupTransform(g);});
document.getElementById('scene-group-scale').addEventListener('input',e=>{
  const g=getSelectedSceneGroup(); if(!g) return;
  g.scale=sliderPosToScale(+e.target.value,SCENEOBJ_SCALE_MIN,SCENEOBJ_SCALE_MAX);
  applySceneGroupTransform(g);
  document.getElementById('scene-group-scale-val').textContent=g.scale.toFixed(2);
});
document.getElementById('scene-group-vis-on').addEventListener('click',()=>{
  const g=getSelectedSceneGroup(); if(!g) return;
  g.root.visible=true; updateSceneGroupDisplays(); renderSceneGroupsList(); updateShadowCameraFrustum();
});
document.getElementById('scene-group-vis-off').addEventListener('click',()=>{
  const g=getSelectedSceneGroup(); if(!g) return;
  g.root.visible=false; updateSceneGroupDisplays(); renderSceneGroupsList(); updateShadowCameraFrustum();
});
document.getElementById('scene-group-reset').addEventListener('click',()=>{
  const g=getSelectedSceneGroup(); if(!g) return;
  g.posX=g.posY=g.posZ=0; g.rotX=g.rotY=g.rotZ=0; g.scale=1;
  applySceneGroupTransform(g); updateSceneGroupDisplays();
});
document.getElementById('scene-group-ungroup-btn').addEventListener('click',()=>{
  const g=getSelectedSceneGroup(); if(!g) return;
  ungroupSceneGroup(g.id);
});
document.getElementById('scene-group-delete-btn').addEventListener('click',()=>{
  const g=getSelectedSceneGroup(); if(!g) return;
  if(confirm(`Delete "${g.name}" and all ${sceneObjects.filter(o=>o.groupId===g.id).length} member object(s)?`)) deleteSceneGroup(g.id);
});
...
document.querySelectorAll('[data-soaaxis]').forEach(btn=>btn.addEventListener('click',()=>{
  const obj=getSelectedSceneObj(); if(!obj) return;
  const dir=+btn.dataset.sodir, ax=btn.dataset.soaaxis;
  if(ax==='x') obj.anchorX=+(((obj.anchorX||0)+sceneObjStep*dir).toFixed(3));
  if(ax==='y') obj.anchorY=+(((obj.anchorY||0)+sceneObjStep*dir).toFixed(3));
  if(ax==='z') obj.anchorZ=+(((obj.anchorZ||0)+sceneObjStep*dir).toFixed(3));
  applySceneObjTransform(obj); updateSceneObjDisplays();
}));
document.getElementById('so-anchor-x-val').addEventListener('change',e=>{const obj=getSelectedSceneObj();if(!obj)return;obj.anchorX=+e.target.value||0;applySceneObjTransform(obj);});
document.getElementById('so-anchor-y-val').addEventListener('change',e=>{const obj=getSelectedSceneObj();if(!obj)return;obj.anchorY=+e.target.value||0;applySceneObjTransform(obj);});
document.getElementById('so-anchor-z-val').addEventListener('change',e=>{const obj=getSelectedSceneObj();if(!obj)return;obj.anchorZ=+e.target.value||0;applySceneObjTransform(obj);});
document.getElementById('scene-obj-parent-indep').addEventListener('click',()=>{
  const obj=getSelectedSceneObj(); if(!obj) return;
  setSceneObjJoined(obj,false); applySceneObjTransform(obj); updateSceneObjDisplays();
});
document.getElementById('scene-obj-parent-joined').addEventListener('click',()=>{
  const obj=getSelectedSceneObj(); if(!obj) return;
  setSceneObjJoined(obj,true); applySceneObjTransform(obj); updateSceneObjDisplays();
});
document.getElementById('scene-obj-reset').addEventListener('click',()=>{
  ...
  obj.anchorX=obj.anchorY=obj.anchorZ=0;
  applySceneObjTransform(obj); updateSceneObjDisplays();
});
document.getElementById('scene-obj-scale').addEventListener('input',e=>{
  const obj=getSelectedSceneObj(); if(!obj) return;
  obj.scale=sliderPosToScale(+e.target.value,SCENEOBJ_SCALE_MIN,SCENEOBJ_SCALE_MAX);
  applySceneObjTransform(obj);
  document.getElementById('scene-obj-scale-val').textContent=obj.scale.toFixed(2);
});
...
function collectSceneObjectsForSave(){
  return sceneObjects.map(o=>({
    name:o.name, type:o.type||'model', glbBlob:o.glbBlob,
    posX:o.posX,posY:o.posY,posZ:o.posZ,
    rotX:o.rotX,rotY:o.rotY,rotZ:o.rotZ,
    scale:o.scale, selClipIdx:o.selClipIdx,
    animMode:o.animMode, phaseOffset:o.phaseOffset, visible:o.root.visible,
    anchorX:o.anchorX,anchorY:o.anchorY,anchorZ:o.anchorZ, joined:o.joined,
    brightness:o.brightness,contrast:o.contrast,saturation:o.saturation
  }));
}
function collectSceneGroupsForSave(){
  // Member associations are saved by INDEX into the current sceneObjects
  // array, not by id — ids regenerate every time objects are restored, but
  // restoreSceneObjects recreates them in the same order they're saved in,
  // so index position is a stable reference across a save/restore cycle.
  return sceneGroups.map(g=>({
    name:g.name, posX:g.posX,posY:g.posY,posZ:g.posZ,
    rotX:g.rotX,rotY:g.rotY,rotZ:g.rotZ,scale:g.scale,
    visible:g.root.visible,
    memberIndices: sceneObjects.map((o,i)=>o.groupId===g.id?i:-1).filter(i=>i>=0)
  }));
}
async function restoreSceneGroups(dataArr){
  sceneGroups.forEach(g=>scene.remove(g.root));
  sceneGroups=[]; groupIdCounter=0; selectedGroupId=null;
  if(dataArr&&dataArr.length){
    for(const d of dataArr){
      const members=(d.memberIndices||[]).map(i=>sceneObjects[i]).filter(Boolean);
      if(!members.length) continue;
      const root=new THREE.Group();
      scene.add(root);
      // Plain .add() (not .attach()) — each member's saved posX/Y/Z etc.
      // were already relative to the group at save time, so they should
      // carry over unchanged, just reinterpreted under the new parent.
      members.forEach(o=>root.add(o.root));
      const id=groupIdCounter++;
      const group={
        id, name:d.name||`Group ${sceneGroups.length+1}`, root,
      ...
      root.visible=d.visible??true;
      members.forEach(o=>{ o.groupId=id; });
      applySceneGroupTransform(group);
      sceneGroups.push(group);
    }
  }
  renderSceneObjList();
...
async function restoreSceneObjects(dataArr){
  ...
  for(const d of dataArr){
    try{
      ...
      const wrap=normalizeSceneObject(gltf.scene);
      wrap.visible=d.visible??true;
      scene.add(wrap);
      const objClips=gltf.animations||[];
      const objMixer=objClips.length?new THREE.AnimationMixer(gltf.scene):null;
      const id=sceneObjIdCounter++;
      const obj={
        id,name:d.name,type:d.type||(isImageFile(d.name)?'image':'model'),glbBlob:d.glbBlob,root:wrap,clips:objClips,mixer:objMixer,
        selClipIdx:d.selClipIdx??(objClips.length?0:-1),
        animMode:d.animMode??'independent', phaseOffset:d.phaseOffset??0,
        posX:d.posX??0,posY:d.posY??0,posZ:d.posZ??0,
        rotX:d.rotX??0,rotY:d.rotY??0,rotZ:d.rotZ??0,scale:d.scale??1,
        anchorX:d.anchorX??0,anchorY:d.anchorY??0,anchorZ:d.anchorZ??0,joined:d.joined??false,groupId:null,
        brightness:d.brightness??0,contrast:d.contrast??1,saturation:d.saturation??1
      };
      setupColorGradeForObject(obj);
      if(obj.joined) setSceneObjJoined(obj,true);
      applySceneObjTransform(obj);
...
document.getElementById('px-dither-off').addEventListener('click',()=>{pixelDither=false;document.getElementById('px-dither-off').classList.add('on');document.getElementById('px-dither-on').classList.remove('on');});
document.getElementById('px-dither-on').addEventListener('click',()=>{pixelDither=true;document.getElementById('px-dither-on').classList.add('on');document.getElementById('px-dither-off').classList.remove('on');});
...
function renderFrameAtRes(width, height, opts={}){
  ...
  r.shadowMap.enabled=true; // always on — the live WYSIWYG preview specifically needs to show real shadows, not a "fast" approximation without them
  ...
  const savedGizmo=axesGizmo.visible; axesGizmo.visible=false;
  r.render(scene,bc);
  ...
  scene.background=savedBG; grid.visible=savedGrid; anchorMarker.visible=savedMarker; boneMarker.visible=savedBoneMarker; axesGizmo.visible=savedGizmo;
  return pix;
}
...
function updateLivePreview(){
  if(!loadedScene||!previewMode) return;
  ...
  const pix=renderFrameAtRes(pw,ph);
...
// BAKE
// ══════════════════════════════════════════════════════════════════════════
// FPS mode: derives frame count from the clip's ACTUAL duration instead of
// a fixed total — this is what fixes "same 60 for idle and strafe gives
// opposite results." A fixed frame COUNT means the effective sampling rate
// depends entirely on how long the clip happens to be: 60 frames across a
// long idle loop undersamples it (looks chopped), while 60 frames across a
// short strafe loop oversamples it (far more frames than needed for that
// duration, so it reads as slow once played back). Specifying an FPS
// instead keeps the sampling rate CONSTANT regardless of clip length.
// ANIM SPEED folds in here too: doubling speed halves how many frames are
// needed to represent the same motion at the same output fps, so it
// changes N directly rather than needing separate playback-time math.
function computeBakeFrameCount(durationSeconds, hasClip){
  if(framesMode==='count' || !hasClip){
    return Math.min(256,Math.max(1,+document.getElementById('n-frames').value||12));
  }
  const effectiveDuration=Math.max(0.01, durationSeconds/Math.max(0.05,animSpeed));
  return Math.min(256,Math.max(1,Math.round(targetFps*effectiveDuration)));
}
function updateTargetFpsReadout(){
  const readout=document.getElementById('target-fps-readout');
  if(!readout) return;
  if(framesMode!=='fps'){ readout.textContent=''; return; }
  let duration=1, hasClip=false;
  if(blendEnabled && blendLowerFilteredClip){ duration=blendLowerFilteredClip.duration; hasClip=true; }
  else if(selClipIdx>=0 && clips[selClipIdx]){ duration=clips[selClipIdx].duration; hasClip=true; }
  const n=computeBakeFrameCount(duration, hasClip);
  readout.textContent = hasClip ? `= ${n} frames (${duration.toFixed(2)}s clip)` : '(select an animation)';
}
...
async function bake(){
  ...
  if(bakeMode==='manual'){
    ...
    const bakeDuration=bakeBlend?lowerClipForBake.duration:(clip?clip.duration:1);
    N=computeBakeFrameCount(bakeDuration, bakeBlend||!!clip);
  }
  ...
  const wasGizmoVisible=axesGizmo.visible; axesGizmo.visible=false;
  setShadowQuality(1024); // full resolution — this is the one place it's worth the cost, since it only happens once
    ...
    for(let i=0;i<N;i++){
      if(bakeBlend&&mixer){
        ...
        if(window.__BAKE_DEBUG__){
          ...
          const nodeName=dot>=0?trackName.slice(0,dot):trackName;
          const propPath=dot>=0?trackName.slice(dot+1):'';
          const propKey=propPath.split('[')[0];
          const targetNode=loadedScene.getObjectByName(nodeName);
          const propVal=targetNode?JSON.stringify(targetNode[propKey]):'(node not found: "'+nodeName+'")';
          console.log(`[bake] frame ${i}/${N}  t=${t.toFixed(4)}  action.time=${a.time.toFixed(4)}  clip.duration=${clip.duration.toFixed(4)}  tracks=${clip.tracks.length}  track0="${trackName}"  ${propKey}=${propVal}`);
        }
      }
  ...
  axesGizmo.visible=wasGizmoVisible;
  ...
  sheet.toBlob(async (blob)=>{
    try{
      ...
      console.error('Result export setup failed:',e);
      alert('Failed to prepare share/download: '+(e.message||e));
    }
  },'image/png');

  startPlayer(pFrames,fps);
  document.getElementById('result-overlay').classList.add('show');
  setShadowQuality(512); // back to the cheap baseline now that the one-time full-res render is done
...
function applyFilter(pix,w,h){
  switch(filterType){
    case 'pixel':      return filterPixelArt(pix,w,h,pixelSize,pixelLevels,pixelOutline,pixelDither);
...
function buildMedianCutPalette(samplePixels, numColors){
  if(!samplePixels.length) return [[128,128,128]];
  let buckets=[samplePixels];
...
const PA_BAYER4=[0,8,2,10,12,4,14,6,3,11,1,9,15,7,13,5].map(v=>(v/16-0.5));

function filterPixelArt(pix,srcW,srcH,tH,numColors,outline,dither){
  const ar=srcW/srcH, tW=Math.max(1,Math.round(tH*ar));
  const xR=srcW/tW, yR=srcH/tH;

  // 1) Nearest-neighbour downsample — pick ONE source pixel per output cell
  //    (the block's center), never blend/average several together. This is
  //    the single biggest factor in "authentic pixel art" vs. a blurry
  //    downscale: area-averaging blends adjacent colors into soft
  //    transitional tones at every edge, which is the opposite of pixel
  //    art's hard, discrete color boundaries.
  const nn=new Uint8ClampedArray(tW*tH*4);
  for(let ty=0;ty<tH;ty++) for(let tx=0;tx<tW;tx++){
    const sx=Math.min(srcW-1,Math.floor((tx+0.5)*xR));
    const sy=Math.min(srcH-1,Math.floor((ty+0.5)*yR));
    const si=(sy*srcW+sx)*4, di=(ty*tW+tx)*4;
    nn[di]=pix[si]; nn[di+1]=pix[si+1]; nn[di+2]=pix[si+2]; nn[di+3]=pix[si+3];
  }

  // 2) Build ONE shared palette via median-cut, sampled from the full-res
  //    source (more representative than sampling only the already-thinned
  //    nearest-neighbour result) — this is what makes it look like an
  //    artist chose a deliberate small color set, instead of independently
  //    rounding each R/G/B channel (which can still reach thousands of
  //    technically-distinct combinations).
  const samples=[];
  const step=Math.max(1,Math.floor(Math.sqrt((srcW*srcH)/4000)));
  for(let sy=0;sy<srcH;sy+=step) for(let sx=0;sx<srcW;sx+=step){
    const i=(sy*srcW+sx)*4;
    if(pix[i+3]>=64) samples.push([pix[i],pix[i+1],pix[i+2]]);
  }
  const palette=buildMedianCutPalette(samples,numColors);

  // 3) Map every downsampled pixel to its nearest palette color. Optional
  //    ordered (Bayer) dithering perturbs the color slightly before lookup,
  //    in a spatial pattern — so gradients near a palette boundary dither
  //    between the two nearest colors rather than banding hard.
  const out=new Uint8ClampedArray(tW*tH*4);
  for(let ty=0;ty<tH;ty++) for(let tx=0;tx<tW;tx++){
    const di=(ty*tW+tx)*4;
    if(nn[di+3]<64){ out[di]=out[di+1]=out[di+2]=out[di+3]=0; continue; }
    let r=nn[di],g=nn[di+1],b=nn[di+2];
    if(dither){
      const off=PA_BAYER4[(ty%4)*4+(tx%4)]*32;
      r=Math.min(255,Math.max(0,r+off)); g=Math.min(255,Math.max(0,g+off)); b=Math.min(255,Math.max(0,b+off));
    }
    const idx=nearestPaletteIdx(r,g,b,palette);
    out[di]=palette[idx][0]; out[di+1]=palette[idx][1]; out[di+2]=palette[idx][2]; out[di+3]=255;
  }

  // 4) Dark 1px outline at the silhouette edge — applied AFTER palette
  //    mapping so it stays crisp rather than getting quantized away.
  if(outline){
    for(let ty=0;ty<tH;ty++) for(let tx=0;tx<tW;tx++){
...
let dbPromise=null;
// ══════════════════════════════════════════════════════════════════════════
// MINIMAL ZIP — self-contained writer/reader, STORED (uncompressed) entries
// only. No external library/CDN dependency on purpose: this is the format
// used for project export/import, which exists specifically so projects
// survive things like a cache-clear wiping IndexedDB — it needs to work
// fully offline and not depend on any network fetch succeeding.
// ══════════════════════════════════════════════════════════════════════════
function crc32(buf){
  let crc = 0xFFFFFFFF;
...
function zipDosDateTime(date){
  ...
  const d = (((Math.max(1980,date.getFullYear())-1980)&0x7F)<<9) | (((date.getMonth()+1)&0xF)<<5) | (date.getDate()&0x1F);
  return {time, d};
}
function buildZip(files){ // files: [{name, data: Uint8Array}]
  ...
  const centralParts = [];
  let offset = 0;
  const {time, d} = zipDosDateTime(new Date());
  for(const f of files){
    ...
    lv.setUint16(8, 0, true); // 0 = stored, no compression
...
function parseZip(arrayBuffer){
  ...
  for(let i=0;i<numEntries;i++){
    ...
    if(compression !== 0) throw new Error('Unsupported compressed entry in project file: '+name);
...
function collectSettings(){
  return {
    ...
    framesMode,targetFps,animSpeed,
    posOffX,posOffY,posOffZ,posStep,primaryScale,primaryModelVisible,
    rotYaw,rotPitch,rotRoll,snapStep,
    anchorX,anchorY,anchorZ,anchorStep,
    isOrtho,bgIdx,
    layoutMode,layoutCustomCols,
    filterType,
    pixelSize,pixelLevels,pixelOutline,pixelDither,
    ...
    camMode,camStep,savedShots,camRotateSpeed:controls.rotateSpeed,
    camPosX:cam.position.x,camPosY:cam.position.y,camPosZ:cam.position.z,
    camTgtX:controls.target.x,camTgtY:controls.target.y,camTgtZ:controls.target.z,
    camZoom:cam.zoom,
    lightPreset,lightAmbient,lightSun,lightFill,lightRim,shadowsOn,groundShadowEnabled,groundShadowOpacity,
    sunAzimuth,sunElevation,sunRadius,fillAzimuth,fillElevation,fillRadius,lightRotStep
  };
}
function applySettings(s){
  ...
  framesMode=s.framesMode??'fps'; targetFps=s.targetFps??30; animSpeed=s.animSpeed??1;
  document.getElementById('frames-mode-count').classList.toggle('on',framesMode==='count');
  document.getElementById('frames-mode-fps').classList.toggle('on',framesMode==='fps');
  document.getElementById('frames-row').style.display=framesMode==='count'?'':'none';
  document.getElementById('target-fps-row').style.display=framesMode==='fps'?'':'none';
  document.getElementById('target-fps').value=targetFps;
  document.getElementById('anim-speed-slider').value=animSpeed;
  document.getElementById('anim-speed-val').textContent=animSpeed.toFixed(2)+'×';
  if(mixer) mixer.timeScale=animSpeed;
  posOffX=s.posOffX??0; posOffY=s.posOffY??0; posOffZ=s.posOffZ??0; posStep=s.posStep??0.25;
  primaryScale=s.primaryScale??1;
  setPrimaryModelVisible(s.primaryModelVisible??true);
  ...
  pixelSize=s.pixelSize??48; pixelLevels=s.pixelLevels??16; pixelOutline=s.pixelOutline??false; pixelDither=s.pixelDither??false;
  ...
  camMode=s.camMode??'fly'; camStep=s.camStep??0.5;
  controls.rotateSpeed=s.camRotateSpeed??1.0;
  document.getElementById('cam-rotate-speed').value=controls.rotateSpeed;
  document.getElementById('cam-rotate-speed-val').textContent=controls.rotateSpeed.toFixed(2);
  savedShots=s.savedShots??[];
  shotIdCounter=savedShots.reduce((m,sh)=>Math.max(m,sh.id+1),0);
  lightPreset=s.lightPreset??'studio';
  lightAmbient=s.lightAmbient??0.55; lightSun=s.lightSun??1.15;
  lightFill=s.lightFill??0.35; lightRim=s.lightRim??0.18;
  shadowsOn=s.shadowsOn??true;
  groundShadowEnabled=s.groundShadowEnabled??false;
  groundShadowOpacity=s.groundShadowOpacity??0.4;
  groundShadowCatcher.material.opacity=groundShadowOpacity;
  groundShadowCatcher.visible=groundShadowEnabled;
  document.getElementById('ground-shadow-on-btn').classList.toggle('on',groundShadowEnabled);
  document.getElementById('ground-shadow-off-btn').classList.toggle('on',!groundShadowEnabled);
  document.getElementById('ground-shadow-opacity-row').style.display=groundShadowEnabled?'':'none';
  document.getElementById('ground-shadow-opacity').value=groundShadowOpacity;
  document.getElementById('ground-shadow-opacity-val').textContent=groundShadowOpacity.toFixed(2);
  sunAzimuth=s.sunAzimuth??39; sunElevation=s.sunElevation??55; sunRadius=s.sunRadius??11;
  fillAzimuth=s.fillAzimuth??-143; fillElevation=s.fillElevation??31; fillRadius=s.fillRadius??5.8;
  lightRotStep=s.lightRotStep??15;
  applyLighting();

  // Restore camera framing (position/target/zoom/type) exactly as left,
  // rather than the generic auto-fit — this is the whole point of the rig.
  if(s.isOrtho!==(cam===orthoCam)){
    isOrtho=s.isOrtho??true; cam=isOrtho?orthoCam:perspCam;
    controls.object=cam;
    document.getElementById('cam-btn').textContent=isOrtho?'ORTHO':'PERSP';
    document.getElementById('cam-btn').classList.toggle('on',!isOrtho);
  } else {
    isOrtho=s.isOrtho??true;
  }
  if(s.camPosX!=null) cam.position.set(s.camPosX,s.camPosY,s.camPosZ);
  if(s.camTgtX!=null) controls.target.set(s.camTgtX,s.camTgtY,s.camTgtZ);
  if(s.camZoom!=null){ cam.zoom=s.camZoom; cam.updateProjectionMatrix(); }
  controls.enabled = camMode!=='locked';
  controls.update(); syncOrtho();
...
function syncUIFromState(){
  ...
  document.getElementById('ground-shadow-opacity').value=groundShadowOpacity;
  document.getElementById('ground-shadow-opacity-val').textContent=groundShadowOpacity.toFixed(2);
  document.getElementById('primary-vis-on').classList.toggle('on',primaryModelVisible);
  document.getElementById('primary-vis-off').classList.toggle('on',!primaryModelVisible);
  document.getElementById('light-preset-studio').classList.toggle('on',lightPreset==='studio');
  document.getElementById('light-preset-flat').classList.toggle('on',lightPreset==='flat');
  document.getElementById('light-preset-custom').classList.toggle('on',lightPreset==='custom');
  syncBtnGroup('[data-lrstep]','lrstep',lightRotStep);
  updateLightDisplays();
  document.getElementById('cam-mode-fly').classList.toggle('on',camMode==='fly');
  document.getElementById('cam-mode-locked').classList.toggle('on',camMode==='locked');
  document.getElementById('cam-locked-controls').style.display=camMode==='locked'?'':'none';
  syncBtnGroup('[data-camstep]','camstep',camStep);
  ...
  document.getElementById('px-dither-on').classList.toggle('on',pixelDither);
  document.getElementById('px-dither-off').classList.toggle('on',!pixelDither);
...
// ══════════════════════════════════════════════════════════════════════════
// PROJECT EXPORT / IMPORT — saves a project as a portable .zip file (model +
// attachments + scene objects/groups + all settings), independent of
// IndexedDB. This exists specifically so a project survives things that
// wipe IndexedDB — clearing Website Data (which this app's own
// troubleshooting steps have called for repeatedly), switching devices, or
// a fresh install after an update. Export/import work on ANY version of the
// app going forward, since every restore path already defaults missing
// fields via `??` for backward compatibility.
// ══════════════════════════════════════════════════════════════════════════
async function blobBytes(blob){ return new Uint8Array(await blob.arrayBuffer()); }
async function buildProjectFilesForZip(rec){
  const files=[];
  const manifest={
    formatVersion:1, exportedAt:new Date().toISOString(),
    name:rec.name, glbName:rec.glbName,
    settings:rec.settings, sceneGroupsData:rec.sceneGroupsData||[],
    attachments:[], sceneObjects:[]
  };
  const primaryExt=detectFormat(rec.glbName||'')||'glb';
  const primaryFile=`primary.${primaryExt}`;
  manifest.primaryFile=primaryFile;
  files.push({name:primaryFile, data:await blobBytes(rec.glbBlob)});

  const attachData=rec.attachmentsData||[];
  for(let i=0;i<attachData.length;i++){
    const a=attachData[i];
    const ext=detectFormat(a.name||'')||'glb';
    const fname=`attachment_${i}.${ext}`;
    const {glbBlob,...rest}=a;
    manifest.attachments.push({...rest, file:fname});
    files.push({name:fname, data:await blobBytes(a.glbBlob)});
  }
  const sceneObjData=rec.sceneObjectsData||[];
  for(let i=0;i<sceneObjData.length;i++){
    const o=sceneObjData[i];
    const ext=detectFormat(o.name||'')||'glb';
    const fname=`sceneobj_${i}.${ext}`;
    const {glbBlob,...rest}=o;
    manifest.sceneObjects.push({...rest, file:fname});
    files.push({name:fname, data:await blobBytes(o.glbBlob)});
  }
  files.push({name:'project.json', data:new TextEncoder().encode(JSON.stringify(manifest))});
  return files;
}
async function exportProjectRecord(rec){
  const files=await buildProjectFilesForZip(rec);
  const zipBlob=buildZip(files);
  const zipName=`${sanitizeZipEntryName(rec.name||'project')}.spritebaker.zip`;
  ...
  if(navigator.canShare){
    ...
    if(navigator.canShare({files:[zipFile]})) navigator.share({files:[zipFile],title:zipName}).catch(()=>{});
  }
}
async function exportCurrentProject(){
  ...
  try{
    const rec={
      name:loadedName||'project', glbBlob:currentGlbBlob, glbName:loadedName,
      settings:collectSettings(), attachmentsData:collectAttachmentsForSave(),
      sceneObjectsData:collectSceneObjectsForSave(), sceneGroupsData:collectSceneGroupsForSave()
    };
    await exportProjectRecord(rec);
  }catch(e){
    console.error('exportCurrentProject failed:',e);
    alert('Failed to export project: '+(e.message||e));
  }
}
async function exportSavedProject(id){
  try{
    const rec=await loadProjectDB(id);
    if(!rec){ alert('Project not found.'); return; }
    await exportProjectRecord(rec);
  }catch(e){
    console.error('exportSavedProject failed:',e);
    alert('Failed to export project: '+(e.message||e));
  }
}
async function importProjectFile(file){
  try{
    ...
    const manifestBytes=zipFiles['project.json'];
    if(!manifestBytes) throw new Error('Not a valid project file (missing project.json)');
    const manifest=JSON.parse(new TextDecoder().decode(manifestBytes));
    const primaryBytes=zipFiles[manifest.primaryFile];
    if(!primaryBytes) throw new Error('Project file is missing its primary model data');
    const primaryBlob=new Blob([primaryBytes]);
    const attachmentsData=(manifest.attachments||[]).map(a=>{
      const bytes=zipFiles[a.file];
      return {...a, glbBlob: bytes?new Blob([bytes]):null};
    });
    const sceneObjectsData=(manifest.sceneObjects||[]).map(o=>{
      const bytes=zipFiles[o.file];
      return {...o, glbBlob: bytes?new Blob([bytes]):null};
    });
    loadFile(primaryBlob, manifest.glbName||manifest.name||'model.glb');
    const check=setInterval(()=>{
      if(loadedScene){
        clearInterval(check);
        applySettings(manifest.settings);
        restoreAttachments(attachmentsData);
        restoreSceneObjects(sceneObjectsData).then(()=>restoreSceneGroups(manifest.sceneGroupsData||[]));
      }
    },50);
    closeProjectsOverlay();
  }catch(e){
    console.error('importProjectFile failed:',e);
    alert('Failed to import project: '+(e.message||e));
  }
}
async function saveProjectAs(name){
  ...
  const rec={id, name:id, savedAt:Date.now(), glbBlob:currentGlbBlob, glbName:loadedName, settings:collectSettings(), attachmentsData:collectAttachmentsForSave(), sceneObjectsData:collectSceneObjectsForSave(), sceneGroupsData:collectSceneGroupsForSave()};
  return new Promise((resolve,reject)=>{
...
async function refreshProjectsList(){
  ...
  projects.forEach(p=>{
    ...
    const expBtn=document.createElement('button'); expBtn.className='tbtn'; expBtn.textContent='⇧';
    expBtn.title='Export as file';
    expBtn.addEventListener('click',e=>{ e.stopPropagation(); exportSavedProject(p.id); });
    ...
    row.appendChild(info); row.appendChild(loadBtn); row.appendChild(expBtn); row.appendChild(delBtn);
    list.appendChild(row);
  });
}
async function loadProjectAndApply(id){
  ...
  const check=setInterval(()=>{
    if(loadedScene){
      ...
      restoreSceneObjects(rec.sceneObjectsData).then(()=>restoreSceneGroups(rec.sceneGroupsData));
    }
  },50);
  closeProjectsOverlay();
}
...
document.getElementById('export-current-btn').addEventListener('click', exportCurrentProject);
document.getElementById('import-proj-input').addEventListener('change', e=>{
  const f=e.target.files[0]; if(f) importProjectFile(f); e.target.value='';
});
