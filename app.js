...
// ══════════════════════════════════════════════════════════════════════════
// Hides only the primary model's MESH geometry, not the bone/group hierarchy
// itself — bones aren't rendered anyway, so any weapon attachment parented to
// one stays fully visible and correctly posed even with the character hidden.
// Simply hiding the whole pivot would take attachments down with it, since
// they're deep children of it via the skeleton.
function setPrimaryModelVisible(visible){
  if(!loadedScene) return;
  loadedScene.traverse(n=>{
    if(n.isMesh||n.isSkinnedMesh) n.visible=visible;
  });
}
...
function renderAttachmentsList(){
  ...
  attachments.forEach(att=>{
    ...
    vis.addEventListener('click',e=>{ e.stopPropagation(); att.root.visible=!att.root.visible; renderAttachmentsList(); });
    row.appendChild(vis);
    const del=document.createElement('button'); del.className='tbtn'; del.textContent='DEL';
...
function renderSceneObjList(){
  ...
  sceneObjects.forEach(obj=>{
    ...
    vis.addEventListener('click',e=>{ e.stopPropagation(); obj.root.visible=!obj.root.visible; renderSceneObjList(); });
    row.appendChild(vis);
    const del=document.createElement('button'); del.className='tbtn'; del.textContent='DEL';
