...
- **Lighting** — the scene has a 3-point rig (ambient + directional "sun" with shadows + a cool fill + a warm rim) by default (STUDIO preset). FLAT switches to ambient-only lighting with shadows off — genuinely flat, since ambient light isn't affected by surface normals at all, so there's no highlight/shadow gradient. Right for ground/road tiles that shouldn't show a sun hotspot. Individual AMBIENT/SUN/FILL/RIM sliders and a separate SHADOWS toggle give full manual control (CUSTOM) for anything in between. SUN and FILL can also be rotated (azimuth/elevation steppers) to move the highlight/shadow direction rather than just dialing intensity. The sun's shadow camera frustum automatically resizes to fit whatever's currently in the scene (primary model + visible scene objects) — without this, scaling the primary model very small or very large, or moving it far from center, would either produce shadow-acne noise or drop the model out of shadow range entirely.
