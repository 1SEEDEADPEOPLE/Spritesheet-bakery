...
- **Camera rig** — FLY mode is the default free-orbit drag/pinch/scroll behavior. LOCKED mode disables dragging and swaps in precise numeric steppers for camera position and look-at target, plus a zoom slider, so you can fine-tune framing without an accidental finger nudge undoing it. **📷 SAVE CAMERA SHOT** bookmarks the current position/target/zoom/camera-type under a name — tap any saved shot to jump back to it exactly, so zooming in to inspect something small no longer means losing the wider framing you had. Saved shots and the current camera framing both persist with the project.
